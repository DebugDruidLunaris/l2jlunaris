...
public class _178_IconicTrinity extends Quest implements ScriptFile
{
	...
	public String onEvent(String event, QuestState st, NpcInstance npc)
	{
		...
		else if(event.equalsIgnoreCase("reply_2") && GetMemoState == 10 && st.getPlayer().getLevel() <= 20 && st.getPlayer().getClassId().getLevel() == 1)
		{
		...
		else if(event.equalsIgnoreCase("reply_4") && GetMemoState == 10 && (st.getPlayer().getLevel() > 20 || st.getPlayer().getClassId().getLevel() != 1))
		{
			st.giveItems(scrl_of_ench_am_d, 1, true);
			st.soundEffect(SOUND_FINISH);
			st.exitQuest(false);
		...
		{
			htmltext = HtmCache.getInstance().getNotNull("quests/_178_IconicTrinity/past_statue_q0178_11.htm", st.getPlayer());
		...
		else if(event.equalsIgnoreCase("reply_13") && GetMemoState == 3)
		{
			...
			htmltext = HtmCache.getInstance().getNotNull("quests/_178_IconicTrinity/past_statue_q0178_13.htm", st.getPlayer());
			htmltext = htmltext.replace("<?name1?>", st.getPlayer().getName());
		...
		{
			htmltext = HtmCache.getInstance().getNotNull("quests/_178_IconicTrinity/present_statue_q0178_11.htm", st.getPlayer());
			htmltext = htmltext.replace("<?name1?>", st.getPlayer().getName());
		}
		else if(event.equalsIgnoreCase("reply_12a"))
		{
			htmltext = HtmCache.getInstance().getNotNull("quests/_178_IconicTrinity/present_statue_q0178_12.htm", st.getPlayer());
			htmltext = htmltext.replace("<?name1?>", st.getPlayer().getName());
		}
		else if(event.equalsIgnoreCase("reply_13a"))
		{
			htmltext = HtmCache.getInstance().getNotNull("quests/_178_IconicTrinity/present_statue_q0178_13.htm", st.getPlayer());
			htmltext = htmltext.replace("<?name1?>", st.getPlayer().getName());
		}
		else if(event.equalsIgnoreCase("reply_14a") && GetMemoState == 6)
		{
			...
			htmltext = HtmCache.getInstance().getNotNull("quests/_178_IconicTrinity/present_statue_q0178_14.htm", st.getPlayer());
			htmltext = htmltext.replace("<?name1?>", st.getPlayer().getName());
			st.soundEffect(SOUND_MIDDLE);
		}
		...
		else if(event.equalsIgnoreCase("reply_11aa") && GetMemoState == 9)
		{
			...
			htmltext = HtmCache.getInstance().getNotNull("quests/_178_IconicTrinity/future_statue_q0178_12.htm", st.getPlayer());
			htmltext = htmltext.replace("<?name1?>", st.getPlayer().getName());
			st.soundEffect(SOUND_MIDDLE);
		}
		return htmltext;
			...
			case STARTED:
				if(npcId == kekrops)
				{
					if(GetMemoState == 1 || GetMemoState == 2)
						...
						if(st.getPlayer().getLevel() <= 20 && st.getPlayer().getClassId().getLevel() == 1)
