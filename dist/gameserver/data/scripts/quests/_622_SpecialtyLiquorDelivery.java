...
public class _622_SpecialtyLiquorDelivery extends Quest implements ScriptFile
{
	//NPCs
	...
	public _622_SpecialtyLiquorDelivery()
	{
		super(false);
		addStartNpc(JEREMY);
		addTalkId(LIETTA);
	...
	public String onEvent(String event, QuestState st, NpcInstance npc)
	{
		...
		int cond = st.getCond();
		long SpecialDrink_count = st.ownItemCount(SpecialDrink);
		...
		if(event.equalsIgnoreCase("jeremy_q0622_0104.htm") && _state == CREATED)
		{
			...
			takeDrink(st, 6);
		else if(event.equalsIgnoreCase("jeremy_q0622_0701.htm") && cond == 6 && st.ownItemCount(FeeOfSpecialDrink) >= 5)
			st.setCond(7);
		else if(event.equalsIgnoreCase("warehouse_keeper_lietta_q0622_0801.htm") && cond == 7 && st.ownItemCount(FeeOfSpecialDrink) >= 5)
		{
			st.takeItems(SpecialDrink, -1);
			st.takeItems(FeeOfSpecialDrink, -1);
	...
	public String onTalk(NpcInstance npc, QuestState st)
	{
		...
		int cond = st.getCond();
		long SpecialDrink_count = st.ownItemCount(SpecialDrink);
		long FeeOfSpecialDrink_count = st.ownItemCount(FeeOfSpecialDrink);
