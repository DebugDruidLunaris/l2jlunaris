...
public class _417_PathToScavenger extends Quest implements ScriptFile
{
	...
	public String onEvent(String event, QuestState st, NpcInstance npc)
	{
		...
		if(event.equals("1"))
		{
			...
			if(st.getPlayer().getLevel() >= 18 && st.getPlayer().getClassId().getId() == 0x35 && st.ownItemCount(RING_OF_RAVEN) == 0)
			{
				st.setCond(1);
				st.setState(STARTED);
			...
			else if(st.getPlayer().getLevel() >= 18 && st.getPlayer().getClassId().getId() == 0x35 && st.ownItemCount(RING_OF_RAVEN) == 1)
				htmltext = "collector_pipi_q0417_04.htm";
		}
		else if(event.equals("30519_1"))
		{
			if(st.ownItemCount(PIPIS_LETTER) > 0)
			{
		...
		{
			if(st.ownItemCount(ZIMENFS_POTION, CHARIS_AXE, BRONKS_INGOT) > 0)
			{
		...
		{
			if(st.ownItemCount(ZIMENFS_PAY) > 0 || st.ownItemCount(CHALIS_PAY) > 0 || st.ownItemCount(BRONKS_PAY) > 0)
			{
				int n = Rnd.get(3);
		...
		{
			if(st.ownItemCount(BEAD_PARCEL) > 0)
			{
		...
		else if(event.equals("30316_2"))
		{
			...
			if(st.ownItemCount(ROUTS_TP_SCROLL) > 0)
			{
	...
	public String onTalk(NpcInstance npc, QuestState st)
	{
		...
		if(npcId == Pippi)
		{
			...
			else if(st.ownItemCount(PIPIS_LETTER) == 0 && id == STARTED)
				htmltext = "collector_pipi_q0417_01.htm";
			else if(st.ownItemCount(PIPIS_LETTER) == 0)
		...
		else if(npcId == Mion)
		{
			...
			else if(st.ownItemCount(CHARIS_AXE, BRONKS_INGOT, ZIMENFS_POTION) > 0 && st.getInt("id") / 10 == 0)
				htmltext = "trader_mion_q0417_05.htm";
			else if(st.ownItemCount(CHARIS_AXE, BRONKS_INGOT, ZIMENFS_POTION) > 0 && st.getInt("id") / 10 > 0)
				htmltext = "trader_mion_q0417_08.htm";
			else if(st.ownItemCount(CHALIS_PAY, BRONKS_PAY, ZIMENFS_PAY) > 0 && st.getInt("id") < 50)
		...
		{
			if(st.ownItemCount(CHARIS_AXE) > 0)
			{
				if(st.getInt("id") < 20)
				...
				if(st.getInt("id") >= 50)
					st.setCond(3);
				st.setMemoState("id", st.getInt("id") + 10);
			}
		...
		{
			if(st.ownItemCount(BRONKS_INGOT) == 1)
			{
				if(st.getInt("id") < 20)
		...
		{
			if(st.ownItemCount(ZIMENFS_POTION) == 1)
			{
				if(st.getInt("id") < 20)
		...
		else if(npcId == Toma)
		{
			if(st.ownItemCount(MIONS_LETTER) == 1)
			{
				...
				st.setCond(5);
				st.setMemoState("id", String.valueOf(0));
			}
			...
			else if(st.ownItemCount(BEAR_PIC) == 1 && st.ownItemCount(HONEY_JAR) >= 5)
			{
				...
				st.takeItems(HONEY_JAR, st.ownItemCount(HONEY_JAR));
			...
			else if(st.ownItemCount(TARANTULA_PIC) == 1 && st.ownItemCount(BEAD) >= 20)
			{
				...
				st.takeItems(BEAD, st.ownItemCount(BEAD));
				st.takeItems(TARANTULA_PIC, 1);
				st.giveItems(BEAD_PARCEL, 1);
				st.setCond(9);
			}
			else if(st.ownItemCount(BEAD_PARCEL) > 0)
		...
		else if(npcId == Torai && st.ownItemCount(ROUTS_TP_SCROLL) == 1)
	...
	public String onKill(NpcInstance npc, QuestState st)
	{
		...
		if(npcId == HunterBear)
		{
			if(cond && st.ownItemCount(BEAR_PIC) == 1 && st.ownItemCount(HONEY_JAR) < 5 && Rnd.chance(20))
				st.addSpawn(HoneyBear);
		}
		else if(npcId == HoneyBear)
		{
			if(cond && st.ownItemCount(BEAR_PIC) == 1 && st.ownItemCount(HONEY_JAR) < 5)
				if(mob.isSpoiled())
				{
			...
			if(cond && st.ownItemCount(TARANTULA_PIC) == 1 && st.ownItemCount(BEAD) < 20)
				if(mob.isSpoiled())
					if(Rnd.chance(50))
					{
						st.giveItems(BEAD, 1);
