...
public class _463_IMustBeaGenius extends Quest implements ScriptFile
{
	...
	public String onEvent(String event, QuestState st, NpcInstance npc)
	{
		...
		if(npc.getNpcId() == GUTENHAGEN)
			if(event.equalsIgnoreCase("collecter_gutenhagen_q0463_05.htm"))
			{
				...
				st.setMemoState("number", String.valueOf(_number));
				// Set drop for mobs
				for(int _mob : MOBS)
				{
					...
					if(_rand == 0)
						_rand = 5;
					st.setMemoState(String.valueOf(_mob), String.valueOf(_rand));
				}
				// One with higher chance
				st.setMemoState(String.valueOf(MOBS[Rnd.get(0, MOBS.length - 1)]), String.valueOf(Rnd.get(1, 100)));
			...
			else if(event.equalsIgnoreCase("reward"))
			{
				...
				st.removeMemo("cond");
				st.removeMemo("number");
				for(int _mob : MOBS)
					st.removeMemo(String.valueOf(_mob));
			...
			switch(st.getState())
			{
				...
				case STARTED:
					if(st.getCond() == 1)
						...
						if(st.ownItemCount(COLLECTION) > 0)
						{
	...
	public String onKill(NpcInstance npc, QuestState st)
	{
		if(st.getState() == STARTED && st.getCond() == 1)
		{
			...
			else if(_number < 0 && st.ownItemCount(CORPSE_LOG) + _number > 0)
			{
			...
			if(st.ownItemCount(CORPSE_LOG) >= _day_number)
			{
