...
public abstract class SagasSuperclass extends Quest
{
	...
	public void giveHallishaMark(QuestState st)
	{
		...
		else
		{
			...
			st.setMemoState("Archon", str(ArchonId));
			st.startQuestTimer("Archon Hellisha has despawned", 600000, Archon);
			Archon.setRunning();
			Archon.getAI().notifyEvent(CtrlEvent.EVT_AGGRESSION, new Object[] { st.getPlayer(), 100000 });
			...
			if(st1 != null)
				if(st1.getCond() == 6)
					if(st1.getInt("kills") < 9)
						st1.setMemoState("kills", str(st1.getInt("kills") + 1));
					else
					{
