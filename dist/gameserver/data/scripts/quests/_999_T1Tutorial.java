...
public class _999_T1Tutorial extends Quest implements ScriptFile
{
	...
	public String onEvent(String event, QuestState st, NpcInstance npc)
	{
		...
		if(event.equalsIgnoreCase("TimerEx_NewbieHelper"))
		{
			if(Ex == 0)
			{
				...
				qs.setMemoState("Ex", "1");
			}
			else if(Ex == 3)
			{
				st.playTutorialVoice("tutorial_voice_010a");
				qs.setMemoState("Ex", "4");
			}
			return null;
		}
		...
		else
		{
			...
			htmltext = e.htm;
			if(st.ownItemCount(e.item) > 0 && st.getInt("onlyone") == 0)
			{
				st.addExpAndSp(0, 50);
	...
	public String onFirstTalk(NpcInstance npc, Player player)
	{
		...
		else if(onlyone == 0 && level < 10)
		{
			...
			if(t.npcTyp == 1)
			{
				...
				else if(step == 1 && st.ownItemCount(t.item) == 0 && Ex <= 2)
				{
					if(st.ownItemCount(BLUE_GEM) > 0)
					{
						st.takeItems(BLUE_GEM, st.ownItemCount(BLUE_GEM));
						st.giveItems(t.item, 1);
						st.setMemoState("step", "2");
						qs.setMemoState("Ex", "3");
						st.startQuestTimer("TimerEx_NewbieHelper", 30000);
						qs.setMemoState("ucMemo", "3");
						if(isMage)
						{
							st.playTutorialVoice("tutorial_voice_027");
	...
	public String onKill(NpcInstance npc, QuestState st)
	{
		...
		if(Ex <= 1)
		{
			...
			qs.setMemoState("Ex", "2");
		}
		if(Ex <= 2 && st.ownItemCount(BLUE_GEM) < 1) //TODO: На оффе квестовые итемы дропаются\даются через 3 сек, после убийства моба!
	...
	public static class DropGem extends RunnableImpl
	{
		...
		public void runImpl() throws Exception
		{
			if(_st != null && _npc != null)
			{
				...
				_st.soundEffect(SOUND_TUTORIAL);
			}
		}
	}
