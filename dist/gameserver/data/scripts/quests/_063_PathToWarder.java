...
public class _063_PathToWarder extends Quest implements ScriptFile
{
	...
	public String onTalk(NpcInstance npc, QuestState st)
	{
		...
		if(npcId == Sione)
			...
			else
			{
				...
				if(cond == 3)
					if(st.ownItemCount(OlMahumOrders) < 10 && st.ownItemCount(OlMahumOrganizationChart) < 5)
		...
		{
			if(cond == 4)
				if(st.ownItemCount(GobiesOrders) < 1)
			...
			if(cond == 12)
				if(st.ownItemCount(TaksCapturedSoul) > 0)
				{
	...
	public String onKill(NpcInstance npc, QuestState st)
	{
		...
		if(cond == 2)
		{
			...
			if(st.ownItemCount(OlMahumOrders) > 9 && st.ownItemCount(OlMahumOrganizationChart) > 4)
			{
				st.setCond(3);
