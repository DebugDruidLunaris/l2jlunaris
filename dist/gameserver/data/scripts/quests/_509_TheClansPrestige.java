...
public class _509_TheClansPrestige extends Quest implements ScriptFile
{
	...
	private static final int[][] REWARDS_LIST = {
			{ 0, 0 },
			{ DAIMON_THE_WHITE_EYED, DAIMONS_EYES, 1378 },
			{ HESTIA_GUARDIAN_DEITY, HESTIAS_FAIRY_STONE, 1378 },
			{ PLAGUE_GOLEM, NUCLEUS_OF_LESSER_GOLEM, 1070 },
			{ DEMONS_AGENT_FALSTON, FALSTONS_FANG, 782 } };
	...
	public String onKill(NpcInstance npc, QuestState st)
	{
		...
		if(st.getCond() == 1 && st.getState() == STARTED)
		{
			...
			if(npcId == raid && st.ownItemCount(item) == 0)
			{
				st.giveItems(item, 1);
