...
public class _129_PailakaDevilsLegacy extends Quest implements ScriptFile
{
	...
	public String onTalk(NpcInstance npc, QuestState st)
	{
		...
		{
			if(st.ownItemCount(SWORD) > 0 && st.ownItemCount(KDROP) == 0)
				htmltext = "32508-01.htm";
			if(st.ownItemCount(ENCHSWORD) > 0 && st.ownItemCount(ADROP) == 0)
				htmltext = "32508-01.htm";
			if(st.ownItemCount(SWORD) == 0 && st.ownItemCount(KDROP) > 0)
				htmltext = "32508-05.htm";
			if(st.ownItemCount(ENCHSWORD) == 0 && st.ownItemCount(ADROP) > 0)
				htmltext = "32508-05.htm";
			if(st.ownItemCount(SWORD) == 0 && st.ownItemCount(ENCHSWORD) == 0)
				htmltext = "32508-05.htm";
			if(st.ownItemCount(KDROP) == 0 && st.ownItemCount(ADROP) == 0)
				htmltext = "32508-01.htm";
			...
			if(st.ownItemCount(LASTSWORD) > 0)
				htmltext = "32508-03.htm";
		}
	...
	public String onKill(NpcInstance npc, QuestState st)
	{
		...
		if(npcId == KAMS && st.ownItemCount(KDROP) == 0)
			st.giveItems(KDROP, 1);
		else if(npcId == ALKASO && st.ownItemCount(ADROP) == 0)
