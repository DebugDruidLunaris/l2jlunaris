	...
	public String onEvent(String event, QuestState st, NpcInstance npc)
	{
		...
		else if(event.startsWith("buff"))
		{
			...
			if(st.getInt("spells") < 4)
			{
				...
				st.setMemoState("spells", "" + (st.getInt("spells") + 1));
		...
		switch(npcId)
		{
			...
			case VSWARRIOR2:
				if(st.getInt("stage") == 1)
					st.setMemoState("stage", "2");
				break;
			case VSCOMMAO1:
			case VSCOMMAO2:
				if(st.getInt("stage") == 2)
					st.setMemoState("stage", "3");
				if(st.ownItemCount(SPEAR) > 0 && st.ownItemCount(STAGE1) == 0)
