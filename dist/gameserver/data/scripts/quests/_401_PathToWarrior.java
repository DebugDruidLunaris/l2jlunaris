...
public class _401_PathToWarrior extends Quest implements ScriptFile
{
	...
	public String onEvent(String event, QuestState st, NpcInstance npc)
	{
		...
		if(event.equalsIgnoreCase("401_1"))
		{
			if(st.getPlayer().getClassId().getId() == 0x00)
			{
				...
				{
					if(st.ownItemCount(MEDALLION_OF_WARRIOR_ID) > 0)
		...
		{
			if(st.ownItemCount(EINS_LETTER_ID) == 0)
			{
				st.setCond(1);
				st.setState(STARTED);
	...
	public String onTalk(NpcInstance npc, QuestState st)
	{
		...
		else if(npcId == AURON && st.ownItemCount(EINS_LETTER_ID) > 0)
			htmltext = "ein_q0401_07.htm";
		else if(npcId == AURON && st.ownItemCount(WARRIOR_GUILD_MARK_ID) == 1)
			htmltext = "ein_q0401_08.htm";
		else if(npcId == SIMPLON && st.ownItemCount(EINS_LETTER_ID) > 0)
			htmltext = "trader_simplon_q0401_01.htm";
		else if(npcId == SIMPLON && st.ownItemCount(WARRIOR_GUILD_MARK_ID) > 0)
		{
			if(st.ownItemCount(RUSTED_BRONZE_SWORD1_ID) < 1)
				htmltext = "trader_simplon_q0401_03.htm";
			else if(st.ownItemCount(RUSTED_BRONZE_SWORD1_ID) < 10)
		...
		else if(npcId == SIMPLON && st.ownItemCount(SIMPLONS_LETTER_ID) > 0)
			htmltext = "trader_simplon_q0401_06.htm";
		else if(npcId == AURON && st.ownItemCount(SIMPLONS_LETTER_ID) > 0 && st.ownItemCount(RUSTED_BRONZE_SWORD2_ID) > 0 && st.ownItemCount(WARRIOR_GUILD_MARK_ID) == 0 && st.ownItemCount(EINS_LETTER_ID) == 0)
			htmltext = "ein_q0401_09.htm";
		else if(npcId == AURON && st.ownItemCount(RUSTED_BRONZE_SWORD3_ID) > 0 && st.ownItemCount(WARRIOR_GUILD_MARK_ID) == 0 && st.ownItemCount(EINS_LETTER_ID) == 0)
			if(st.ownItemCount(POISON_SPIDER_LEG2_ID) < 20)
				htmltext = "ein_q0401_12.htm";
			else if(st.ownItemCount(POISON_SPIDER_LEG2_ID) > 19)
			{
	...
	public String onKill(NpcInstance npc, QuestState st)
	{
		...
		{
			if(cond == 2 && st.ownItemCount(RUSTED_BRONZE_SWORD1_ID) < 10)
			{
				st.giveItems(RUSTED_BRONZE_SWORD1_ID, 1);
				if(st.ownItemCount(RUSTED_BRONZE_SWORD1_ID) == 10)
				{
			...
			if(st.ownItemCount(POISON_SPIDER_LEG2_ID) < 20 && st.ownItemCount(RUSTED_BRONZE_SWORD3_ID) == 1 && st.getItemEquipped(Inventory.PAPERDOLL_RHAND) == RUSTED_BRONZE_SWORD3_ID)
			{
