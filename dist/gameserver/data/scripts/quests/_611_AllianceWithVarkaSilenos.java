...
public class _611_AllianceWithVarkaSilenos extends Quest implements ScriptFile
{
	...
	private static void checkMarks(QuestState st)
	{
		if(st.getCond() == 0)
			return;
		if(st.ownItemCount(MARK_OF_VARKA_ALLIANCE5) > 0)
			st.setCond(6);
		else if(st.ownItemCount(MARK_OF_VARKA_ALLIANCE4) > 0)
			st.setCond(5);
		else if(st.ownItemCount(MARK_OF_VARKA_ALLIANCE3) > 0)
			st.setCond(4);
		else if(st.ownItemCount(MARK_OF_VARKA_ALLIANCE2) > 0)
			st.setCond(3);
		else if(st.ownItemCount(MARK_OF_VARKA_ALLIANCE1) > 0)
			st.setCond(2);
		else
			st.setCond(1);
	}
	...
	{
		if(soilder_count > 0 && st.ownItemCount(KB_SOLDIER) < soilder_count)
			return false;
		if(capitan_count > 0 && st.ownItemCount(KB_CAPTAIN) < capitan_count)
			return false;
		if(general_count > 0 && st.ownItemCount(KB_GENERAL) < general_count)
			return false;
		if(other_item > 0 && st.ownItemCount(other_item) < 1)
			return false;
