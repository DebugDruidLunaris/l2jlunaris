	...
	public String onTalk(NpcInstance npc, QuestState st)
	{
		...
		if(npcId == WAHKAN)
		{
			if(cond == 0)
				if(st.getPlayer().getLevel() >= 74)
					if(st.ownItemCount(MARK_OF_KETRA_ALLIANCE1) == 1 || st.ownItemCount(MARK_OF_KETRA_ALLIANCE2) == 1 || st.ownItemCount(MARK_OF_KETRA_ALLIANCE3) == 1 || st.ownItemCount(MARK_OF_KETRA_ALLIANCE4) == 1 || st.ownItemCount(MARK_OF_KETRA_ALLIANCE5) == 1)
					{
						if(st.ownItemCount(DIVINE_STONE_OF_WISDOM) == 0)
		...
		else if(npcId == ASEFA)
		{
			...
			else if(cond == 2 && proval == 1)
			{
				...
				st.setMemoState("proval", "0");
			}
			else if(cond == 3 && st.ownItemCount(STOLEN_GREEN_TOTEM) >= 1)
			{
				htmltext = "shaman_asefa_q0609_04.htm";
				st.takeItems(STOLEN_GREEN_TOTEM, st.ownItemCount(STOLEN_GREEN_TOTEM));
