...
public class _136_MoreThanMeetsTheEye extends Quest implements ScriptFile
{
	...
	public String onTalk(NpcInstance npc, QuestState st)
	{
		...
		else if(npcId == ERRICKIN)
		{
			...
			else if(cond == 4 && st.ownItemCount(Ectoplasm) < 35 && st.getInt("id") == 0)
			{
				st.setCond(3);
				htmltext = "magister_errickin_q0136_03.htm";
			}
			...
			else if(cond == 8 && st.ownItemCount(GlassJaguarCrystal) < 5)
			{
				htmltext = "magister_clayton_q0136_12.htm";
				st.setCond(7);
			}
	...
	public String onKill(NpcInstance npc, QuestState st)
	{
		...
		for(int i = 0; i < DROPLIST_COND.length; i++)
			if(cond == DROPLIST_COND[i][0] && npcId == DROPLIST_COND[i][2])
				...
				{
					long count = st.ownItemCount(DROPLIST_COND[i][4]);
