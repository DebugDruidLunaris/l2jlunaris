...
public class _416_PathToOrcShaman extends Quest implements ScriptFile
{
	...
	public String onTalk(NpcInstance npc, QuestState st)
	{
		...
		if(npcId == Hestui)
		{
			if(st.ownItemCount(MaskOfMedium) != 0)
			{
		...
		else if(npcId == HestuiTotemSpirit && st.getCond() > 0 && (st.ownItemCount(GrizzlyBlood) > 0 || st.ownItemCount(FlameCharm) > 0 || st.ownItemCount(BloodCauldron) > 0 || st.ownItemCount(SpiritNet) > 0 || st.ownItemCount(BoundDurkaSpirit) > 0 || st.ownItemCount(TotemSpiritBlood) > 0 || st.ownItemCount(TatarusLetterOfRecommendation) > 0))
	...
	public String onKill(NpcInstance npc, QuestState st)
	{
		...
		if(st.ownItemCount(KashaBearPelt) != 0 && st.ownItemCount(KashaBladeSpiderHusk) != 0 && st.ownItemCount(FieryEgg1st) != 0)
			st.setCond(2);
		else if(cond == 9 && (npcId == VenomousSpider || npcId == ArachnidTracker))
		{
			if(st.ownItemCount(DurkaParasite) < 8)
			{
				st.giveItems(DurkaParasite, 1);
				st.soundEffect(SOUND_ITEMGET);
			}
			if(st.ownItemCount(DurkaParasite) == 8 || st.ownItemCount(DurkaParasite) >= 5 && Rnd.chance(st.ownItemCount(DurkaParasite) * 10))
