...
// Fixed By Hl4p3x
public class _348_AnArrogantSearch extends Quest implements ScriptFile
{
	...
	public String onEvent(String event, QuestState st, NpcInstance npc)
	{
		...
		{
			st.setCond(29);
			st.ownItemCount(WHITE_FABRIC_2, 10);
		}
		if(event.equals("30864-10a.htm"))
		{
			if(st.getSumQuestItemsCount(WHITE_FABRIC_2) < 10)
				st.giveItems(WHITE_FABRIC_2, 10 - st.ownItemCount(WHITE_FABRIC_2));
			htmltext = "30864-10.htm";
		}
		if(event.equals("30864-10b.htm"))
		{
			if(st.getSumQuestItemsCount(BLOODED_FABRIC) > 1)
			{
				long count = st.takeItems(BLOODED_FABRIC, -1);
				st.giveItems(ADENA_ID, count * 5000); // с потолка
				htmltext = "30864-10.htm";
			}
			else
	...
	public String onTalk(NpcInstance npc, QuestState st)
	{
		...
		if(npcId == HANELLIN)
		{
			...
			else if(cond == 2 && st.getSumQuestItemsCount(SHELL_OF_MONSTERS) == 0)
			...
			else if(cond == 5 && st.getSumQuestItemsCount(BOOK_OF_SAINT) + st.getSumQuestItemsCount(BLOOD_OF_SAINT) + st.getSumQuestItemsCount(BRANCH_OF_SAINT) < 3)
			...
			else if(cond == 22 && st.getSumQuestItemsCount(ANTIDOTE) < 5 && st.getSumQuestItemsCount(HEALING_POTION) < 1)
			...
			else if(cond == 25 && st.getSumQuestItemsCount(BLOODED_FABRIC) < 1)
			{
				if(st.getSumQuestItemsCount(WHITE_FABRIC_1) < 1)
					st.giveItems(WHITE_FABRIC_1, 1);
				htmltext = "30864-07a.htm";
			}
			else if(cond == 26 && st.getSumQuestItemsCount(BLOODED_FABRIC) < 1)
			{
				if(st.getSumQuestItemsCount(WHITE_FABRIC_2) < 1)
					st.giveItems(WHITE_FABRIC_2, 1);
				htmltext = "30864-07a.htm";
			}
			else if(cond == 25 && st.getSumQuestItemsCount(BLOODED_FABRIC) > 0 || cond == 28)
			{
		...
		// Other NPCs follow:
			...
			if(ARK_OWNERS.containsKey(npcId))
			{
				...
				if(st.getSumQuestItemsCount(ARK_OWNERS.get(npcId)[0]) == 1)
				{
				...
				else if(st.getSumQuestItemsCount(ARK_OWNERS.get(npcId)[1]) < 1)
				{
			...
			else if(ARKS.containsKey(npcId))
			{
				...
				if(st.getSumQuestItemsCount(ARKS.get(npcId)[0]) == 0)
				{
				...
				else if(st.getSumQuestItemsCount(ARKS.get(npcId)[2]) >= 1)
				...
				// if you do not have the key (first meeting)
				if(st.getSumQuestItemsCount(FIRST_KEY_OF_ARK) == 0 && st.getInt("angelKillerIsDefeated") == 0)
				{
					st.addSpawn(ANGEL_KILLER, 120000);
					htmltext = "30980-01.htm";
				}
				else if(st.getSumQuestItemsCount(FIRST_KEY_OF_ARK) == 0 && st.getInt("angelKillerIsDefeated") == 1)
				{
	...
	public String onKill(NpcInstance npc, QuestState st)
	{
		...
		if(drop != null)
		{
			...
			int chance = drop[3];
			int take = drop[4];
			if(st.getCond() >= cond && st.getSumQuestItemsCount(item) < max && (take == 0 || st.getSumQuestItemsCount(take) > 0) && Rnd.chance(chance))
			{
				st.giveItems(item, 1);
				st.soundEffect(SOUND_ITEMGET);
				if(take != 0)
					st.takeItems(take, 1);
				if(BLOODED_FABRIC == item && st.getSumQuestItemsCount(BLOODED_FABRIC) >= 30)
				{
