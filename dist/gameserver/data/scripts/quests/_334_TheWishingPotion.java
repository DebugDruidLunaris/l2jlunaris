...
public class _334_TheWishingPotion extends Quest implements ScriptFile
{
	...
	{
		if(st.ownItemCount(AMBER_SCALE_ID) == 1 && st.ownItemCount(WIND_SOULSTONE_ID) == 1 && st.ownItemCount(GLASS_EYE_ID) == 1 && st.ownItemCount(HORROR_ECTOPLASM_ID) == 1 && st.ownItemCount(SILENOS_HORN_ID) == 1 && st.ownItemCount(ANT_SOLDIER_APHID_ID) == 1 && st.ownItemCount(TYRANTS_CHITIN_ID) == 1 && st.ownItemCount(BUGBEAR_BLOOD_ID) == 1)
		{
			st.setCond(4);
			return true;
		}
	...
	public String onEvent(String event, QuestState st, NpcInstance npc)
	{
		...
		{
			if(st.ownItemCount(WISH_POTION_ID) == 0)
			{
				st.takeItems(ALCHEMY_TEXT_ID, -1);
				st.takeItems(SECRET_BOOK_ID, -1);
				if(st.ownItemCount(POTION_RECIPE_1_ID) == 0)
					st.giveItems(POTION_RECIPE_1_ID, 1);
				if(st.ownItemCount(POTION_RECIPE_2_ID) == 0)
					st.giveItems(POTION_RECIPE_2_ID, 1);
				if(st.ownItemCount(MATILDS_ORB_ID) == 0)
					htmltext = "30738-06.htm";
				else
					htmltext = "30738-12.htm";
				st.setCond(3);
			}
			else if(st.ownItemCount(MATILDS_ORB_ID) >= 1 && st.ownItemCount(WISH_POTION_ID) >= 1)
		...
		{
			if(st.ownItemCount(WISH_POTION_ID) >= 1)
	...
	public String onTalk(NpcInstance npc, QuestState st)
	{
		...
		if(npcId == ALCHEMIST_MATILD)
		{
			if(cond == 0)
			{
				...
				else if(st.ownItemCount(MATILDS_ORB_ID) == 0)
					htmltext = "30738-01.htm";
				else if(st.ownItemCount(WISH_POTION_ID) == 0)
				{
					st.setCond(3);
					if(st.ownItemCount(POTION_RECIPE_1_ID) == 0)
						st.giveItems(POTION_RECIPE_1_ID, 1);
			...
			else if(cond == 1 && st.ownItemCount(ALCHEMY_TEXT_ID) == 1)
				htmltext = "30738-04.htm";
			else if(cond == 2)
			{
				if(st.ownItemCount(SECRET_BOOK_ID) == 1 && st.ownItemCount(ALCHEMY_TEXT_ID) == 1)
		...
		{
			if(st.ownItemCount(FORBIDDEN_LOVE_SCROLL_ID) >= 1)
			{
