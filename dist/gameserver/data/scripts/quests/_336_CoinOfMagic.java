...
public class _336_CoinOfMagic extends Quest implements ScriptFile
{
	...
	private static final int[] UNKNOWN = { GraveLich, DoomServant, DoomArcher, DoomKnight,
			//Kookaburra1,
			Kookaburra2,
			Kookaburra3,
			Kookaburra4,
	...
	private String promote(QuestState st)
	{
		...
		else
		{
			...
			if(h == 6)
			{
				...
				st.setMemoState("grade", str(grade - 1));
				if(grade == 3)
					st.setCond(9);
				else if(grade == 2)
					st.setCond(11);
				st.soundEffect(SOUND_FANFARE_MIDDLE);
			}
			else
			{
	...
	public String onTalk(NpcInstance npc, QuestState st)
	{
		...
		if(npcId == SORINT)
		{
			...
			else if(st.ownItemCount(COIN_DIAGRAM) > 0)
			{
				if(st.ownItemCount(KALDIS_COIN) > 0)
				{
					st.takeItems(KALDIS_COIN, -1);
					st.takeItems(COIN_DIAGRAM, -1);
					st.giveItems(MEMBERSHIP_3, 1);
					st.setMemoState("grade", "3");
					st.setCond(4);
					st.soundEffect(SOUND_FANFARE_MIDDLE);
		...
		{
			if(st.ownItemCount(COIN_DIAGRAM) > 0 && grade == 0)
