...
public class _234_FatesWhisper extends Quest implements ScriptFile
{
	...
	public String onEvent(String event, QuestState st, NpcInstance npc)
	{
		...
		if(oldweapon != 0)
			if(st.ownItemCount(oldweapon) >= 1)
			{
				if(st.ownItemCount(CRYSTAL_B) >= 984)
				{
					st.setMemoState("oldweapon", String.valueOf(oldweapon));
	...
	public String onTalk(NpcInstance npc, QuestState st)
	{
		...
		if(npcId == REORIN)
		{
			...
			else if(cond == COND1 && st.ownItemCount(REIRIAS_SOUL_ORB) >= 1)
				htmltext = "31002-05.htm";
			else if(cond == COND2 && st.ownItemCount(HALLATES_INFERNIUM_SCEPTER) >= 1 && st.ownItemCount(KERNONS_INFERNIUM_SCEPTER) >= 1 && st.ownItemCount(GOLCONDAS_INFERNIUM_SCEPTER) >= 1)
				htmltext = "31002-06.htm";
			else if(cond == COND3 && st.ownItemCount(INFERNIUM_VARNISH) >= 1)
				htmltext = "31002-07.htm";
			else if(cond == COND4 && st.ownItemCount(REORINS_HAMMER) >= 1)
				htmltext = "31002-08.htm";
			else if(cond == COND10 && st.ownItemCount(REORINS_MOLD) >= 1)
			{
				st.takeItems(REORINS_MOLD, -1);
				st.setCond(COND11);
				htmltext = "31002-09.htm";
			}
			else if(cond == COND11 && st.ownItemCount(CRYSTAL_B) >= 984)
				htmltext = "31002-10.htm";
			else if(cond == COND12)
				htmltext = "a-grade.htm";
		}
		else if(npcId == CABRIOCOFFER && cond == COND1 && st.ownItemCount(REIRIAS_SOUL_ORB) == 0)
		{
			st.giveItems(REIRIAS_SOUL_ORB, 1, false);
			htmltext = "31027-01.htm";
		}
		else if(npcId == CHEST_HALLATE && cond == COND2 && st.ownItemCount(HALLATES_INFERNIUM_SCEPTER) == 0)
			htmltext = "31030-01.htm";
		else if(npcId == CHEST_KERNON && cond == COND2 && st.ownItemCount(KERNONS_INFERNIUM_SCEPTER) == 0)
			htmltext = "31028-01.htm";
		else if(npcId == CHEST_GOLKONDA && cond == COND2 && st.ownItemCount(GOLCONDAS_INFERNIUM_SCEPTER) == 0)
			htmltext = "31029-01.htm";
		else if(npcId == CLIFF && cond == COND3 && st.ownItemCount(INFERNIUM_VARNISH) == 0)
			htmltext = "30182-01.htm";
		else if(npcId == FERRIS && cond == COND4 && st.ownItemCount(REORINS_HAMMER) == 0)
		{
			st.giveItems(REORINS_HAMMER, 1, false);
			htmltext = "30847-01.htm";
		}
		else if(npcId == ZENKIN && st.ownItemCount(REORINS_MOLD) == 0 && cond == COND5)
			htmltext = "30178-01.htm";
		else if(npcId == KASPAR)
			if(cond == COND6)
				htmltext = "30833-01.htm";
			else if((cond == COND7 || cond == COND9) && st.ownItemCount(RED_PIPETTE_KNIFE) == 1)
			{
				st.setCond(COND10);
				...
				if(st.ownItemCount(BLOODED_FABRIC) >= 30)
				{
					st.setCond(COND10);
				...
				else if(st.ownItemCount(BloodStainedCloth, BLOODED_FABRIC) >= 30)
				{
					st.setCond(COND10);
	...
	public String onKill(NpcInstance npc, QuestState st)
	{
		...
		{
			long count = st.ownItemCount(BloodStainedCloth);
			if(st.ownItemCount(WhiteCloth) > 0 && count < 30 && Rnd.chance(33))
			{
	...
	{
		int cond = st.getCond();
		if((cond == COND7 || cond == COND9) && npc.getNpcId() == BAIUM && st.ownItemCount(PIPETTE_KNIFE) >= 1 && st.ownItemCount(RED_PIPETTE_KNIFE) == 0 && st.getItemEquipped(Inventory.PAPERDOLL_RHAND) == PIPETTE_KNIFE)
		{
			if(Rnd.chance(50))
