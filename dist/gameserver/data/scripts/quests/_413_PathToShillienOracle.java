...
public class _413_PathToShillienOracle extends Quest implements ScriptFile
{
	...
	public String onEvent(String event, QuestState st, NpcInstance npc)
	{
		...
		{
			if(st.getPlayer().getLevel() >= 18 && st.getPlayer().getClassId().getId() == 0x26 && st.ownItemCount(ORB_OF_ABYSS_ID) < 1)
			...
			else if(st.getPlayer().getLevel() >= 18 && st.getPlayer().getClassId().getId() == 0x26 && st.ownItemCount(ORB_OF_ABYSS_ID) > 0)
	...
	public String onTalk(NpcInstance npc, QuestState st)
	{
		...
		if(npcId == SIDRA)
		{
			...
			else if(cond == 7 && st.ownItemCount(ANDARIEL_BOOK_ID) > 0 && st.ownItemCount(GARMIEL_BOOK_ID) > 0)
			{
		...
		{
			if(cond == 1 && st.ownItemCount(SIDRAS_LETTER1_ID) > 0)
				htmltext = "magister_talbot_q0413_01.htm";
			else if(cond == 2)
			{
				if(st.ownItemCount(BLOODY_RUNE1_ID) < 1)
					htmltext = "magister_talbot_q0413_03.htm";
				else if(st.ownItemCount(BLOODY_RUNE1_ID) > 0)
					htmltext = "magister_talbot_q0413_04.htm";
			}
			else if(cond == 3 && st.ownItemCount(BLOODY_RUNE1_ID) > 4)
			{
			...
			if(cond == 4 && st.ownItemCount(PRAYER_OF_ADON_ID) > 0)
				htmltext = "priest_adonius_q0413_01.htm";
			else if(cond == 5 && st.ownItemCount(ASHEN_BONES_ID) < 1)
				htmltext = "priest_adonius_q0413_05.htm";
			else if(cond == 5 && st.ownItemCount(ASHEN_BONES_ID) < 10)
				htmltext = "priest_adonius_q0413_06.htm";
			else if(cond == 6 && st.ownItemCount(ASHEN_BONES_ID) > 9)
			{
			...
			else if(cond == 7 && st.ownItemCount(ANDARIEL_BOOK_ID) > 0)
	...
	public String onKill(NpcInstance npc, QuestState st)
	{
		...
		if(npcId == DARK_SUCCUBUS)
			if(cond == 2 && st.ownItemCount(BLANK_SHEET1_ID) > 0)
			{
				st.giveItems(BLOODY_RUNE1_ID, 1);
				st.takeItems(BLANK_SHEET1_ID, 1);
				if(st.ownItemCount(BLANK_SHEET1_ID) < 1)
				{
		...
		for(int i : ASHEN_BONES_DROP)
			if(npcId == i && cond == 5 && st.ownItemCount(ASHEN_BONES_ID) < 10)
			{
				st.giveItems(ASHEN_BONES_ID, 1);
				if(st.ownItemCount(ASHEN_BONES_ID) > 9)
				{
					st.soundEffect(SOUND_MIDDLE);
					st.setCond(6);
				}
