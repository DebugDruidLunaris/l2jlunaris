...
public class _107_MercilessPunishment extends Quest implements ScriptFile
{
	...
	public String onTalk(NpcInstance npc, QuestState st)
	{
		...
		if(npcId == 30568)
		{
			...
			else if(cond == 1 && st.ownItemCount(HATOSS_ORDER1) > 0)
				htmltext = "urutu_chief_hatos_q0107_04.htm";
			else if(cond == 2 && st.ownItemCount(HATOSS_ORDER1) > 0 && st.ownItemCount(LETTER_TO_HUMAN) == 0)
				htmltext = "urutu_chief_hatos_q0107_04.htm";
			else if(cond == 3 && st.ownItemCount(HATOSS_ORDER1) > 0 && st.ownItemCount(LETTER_TO_HUMAN) >= 1)
			{
				htmltext = "urutu_chief_hatos_q0107_05.htm";
				st.setCond(4);
			}
			else if(cond == 4 && st.ownItemCount(HATOSS_ORDER2) > 0 && st.ownItemCount(LETTER_TO_DARKELF) == 0)
				htmltext = "urutu_chief_hatos_q0107_05.htm";
			else if(cond == 5 && st.ownItemCount(HATOSS_ORDER2) > 0 && st.ownItemCount(LETTER_TO_DARKELF) >= 1)
			{
				htmltext = "urutu_chief_hatos_q0107_08.htm";
				st.setCond(6);
			}
			else if(cond == 6 && st.ownItemCount(HATOSS_ORDER3) > 0 && st.ownItemCount(LETTER_TO_ELF) == 0)
				htmltext = "urutu_chief_hatos_q0107_08.htm";
			else if(cond == 7 && st.ownItemCount(HATOSS_ORDER3) > 0 && st.ownItemCount(LETTER_TO_ELF) + st.ownItemCount(LETTER_TO_HUMAN) + st.ownItemCount(LETTER_TO_DARKELF) == 3)
			{
				...
				if(st.getPlayer().getClassId().getLevel() == 1 && !st.getPlayer().getVarB("p1q3"))
				{
					...
					st.getPlayer().sendPacket(new ExShowScreenMessage(NpcString.ACQUISITION_OF_RACESPECIFIC_WEAPON_COMPLETE, 5000, ScreenMessageAlign.TOP_CENTER, true));					st.giveItems(1060, 100); // healing potion
		...
		else if(npcId == 30580 && cond >= 1 && (st.ownItemCount(HATOSS_ORDER1) > 0 || st.ownItemCount(HATOSS_ORDER2) > 0 || st.ownItemCount(HATOSS_ORDER3) > 0))
		{
			if(cond == 1)
				st.setCond(2);
	...
	public String onKill(NpcInstance npc, QuestState st)
	{
		...
		int cond = st.getCond();
		if(npcId == 27041)
			if(cond == 2 && st.ownItemCount(HATOSS_ORDER1) > 0 && st.ownItemCount(LETTER_TO_HUMAN) == 0)
			{
