...
public class _344_1000YearsEndofLamentation extends Quest implements ScriptFile
{
	...
	public String onEvent(String event, QuestState st, NpcInstance npc)
	{
		...
		long amount = st.ownItemCount(ARTICLES_DEAD_HEROES);
		int cond = st.getCond();
		...
		else if(event.equalsIgnoreCase("30754-06.htm") && cond == 1)
		{
			...
			else
			{
				...
				else
				{
					...
					st.setMemoState("amount", str(amount));
				}
	...
	private boolean rewards(QuestState st, int npcId)
	{
		...
		if(npcId == ORVEN && st.ownItemCount(CRUCIFIX) > 0)
		{
			st.setMemoState("mission", "1");
		...
		else if(npcId == GARVARENTZ && st.ownItemCount(OLD_TOTEM) > 0)
		{
			st.setMemoState("mission", "2");
		...
		else if(npcId == KAIEN && st.ownItemCount(OLD_HILT) > 0)
		{
			st.setMemoState("mission", "3");
		...
		else if(npcId == RODEMAI && st.ownItemCount(OLD_KEY) > 0)
		{
			st.setMemoState("mission", "4");
