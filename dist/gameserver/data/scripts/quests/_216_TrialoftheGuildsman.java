...
public class _216_TrialoftheGuildsman extends Quest implements ScriptFile
{
	...
	public String onEvent(String event, QuestState st, NpcInstance npc)
	{
		...
		if(event.equalsIgnoreCase("valkon_q0216_06.htm"))
		{
			...
			st.setCond(3);
		else if(event.equalsIgnoreCase("valkon_q0216_05.htm") && st.ownItemCount(ADENA_ID) < 2000)
	...
	public String onTalk(NpcInstance npc, QuestState st)
	{
		...
		if(npcId == VALKON)
		{
			...
			else if(cond == 2 && st.ownItemCount(VALKONS_RECOMMEND) > 0)
				htmltext = "valkon_q0216_07.htm";
			else if(st.ownItemCount(ALLTRANS_INSTRUCTIONS) > 0)
		...
		{
			if(cond == 1 && st.ownItemCount(VALKONS_RECOMMEND) > 0)
			{
				htmltext = "blacksmith_alltran_q0216_01.htm";
				st.setCond(2);
			}
			else if(cond == 4 && st.ownItemCount(VALKONS_RECOMMEND) > 0 && st.ownItemCount(MANDRAGORA_BERRY) == 1)
				htmltext = "blacksmith_alltran_q0216_02.htm";
			else if(cond < 6 && st.ownItemCount(ALLTRANS_INSTRUCTIONS) == 1 && st.ownItemCount(JOURNEYMAN_RING) < 7)
				htmltext = "blacksmith_alltran_q0216_04.htm";
			else if(cond == 6 && st.ownItemCount(JOURNEYMAN_RING) == 7)
		...
		else if(npcId == NORMAN && cond >= 5)
		{
			...
			else if(st.ownItemCount(ALLTRANS_INSTRUCTIONS) > 0 && st.ownItemCount(NORMANS_INSTRUCTIONS) > 0 && st.ownItemCount(NORMANS_RECEIPT) > 0)
				htmltext = "warehouse_keeper_norman_q0216_05.htm";
			else if(st.ownItemCount(ALLTRANS_INSTRUCTIONS) > 0 && st.ownItemCount(NORMANS_INSTRUCTIONS) > 0 && st.ownItemCount(DUNINGS_INSTRUCTIONS) > 0)
				htmltext = "warehouse_keeper_norman_q0216_06.htm";
			else if(st.ownItemCount(ALLTRANS_INSTRUCTIONS) > 0 && st.ownItemCount(NORMANS_INSTRUCTIONS) > 0 && st.ownItemCount(DUNINGS_KEY) >= 30)
				htmltext = "warehouse_keeper_norman_q0216_07.htm";
			else if(st.ownItemCount(ALLTRANS_INSTRUCTIONS) > 0 && st.ownItemCount(NORMANS_LIST) > 0)
			{
				if(st.ownItemCount(GRAY_BONE_POWDER) >= 70 && st.ownItemCount(GRANITE_WHETSTONE) >= 70 && st.ownItemCount(RED_PIGMENT) >= 70 && st.ownItemCount(BRAIDED_YARN) >= 70)
				{
					...
					if(st.ownItemCount(JOURNEYMAN_DECO_BEADS) == 7 && st.ownItemCount(JOURNEYMAN_GEM) == 7)
						st.setCond(6);
				}
				else
					htmltext = "warehouse_keeper_norman_q0216_11.htm";
			}
			else if(st.ownItemCount(NORMANS_INSTRUCTIONS) == 0 && st.ownItemCount(NORMANS_LIST) == 0 && st.ownItemCount(ALLTRANS_INSTRUCTIONS) == 1 && (st.ownItemCount(JOURNEYMAN_GEM) > 0 || st.ownItemCount(JOURNEYMAN_RING) > 0))
				htmltext = "warehouse_keeper_norman_q0216_13.htm";
		}
		else if(npcId == DUNING && cond >= 5)
		{
			if(st.ownItemCount(ALLTRANS_INSTRUCTIONS) > 0 && st.ownItemCount(NORMANS_INSTRUCTIONS) > 0 && st.ownItemCount(NORMANS_RECEIPT) > 0)
				htmltext = "blacksmith_duning_q0216_01.htm";
			else if(st.ownItemCount(ALLTRANS_INSTRUCTIONS) > 0 && st.ownItemCount(NORMANS_INSTRUCTIONS) > 0 && st.ownItemCount(DUNINGS_INSTRUCTIONS) > 0)
				htmltext = "blacksmith_duning_q0216_03.htm";
			else if(st.ownItemCount(ALLTRANS_INSTRUCTIONS) > 0 && st.ownItemCount(NORMANS_INSTRUCTIONS) > 0 && st.ownItemCount(DUNINGS_KEY) == 30)
				htmltext = "blacksmith_duning_q0216_04.htm";
			else if(st.ownItemCount(NORMANS_RECEIPT) == 0 && st.ownItemCount(DUNINGS_INSTRUCTIONS) == 0 && st.ownItemCount(DUNINGS_KEY) == 0 && st.ownItemCount(ALLTRANS_INSTRUCTIONS) == 1)
				htmltext = "blacksmith_duning_q0216_01.htm";
		}
		else if(npcId == PINTER && cond >= 5)
			if(st.ownItemCount(ALLTRANS_INSTRUCTIONS) > 0 && st.ownItemCount(ALLTRANS_RECOMMEND2) > 0)
				if(st.getPlayer().getLevel() < 36)
					htmltext = "blacksmith_pinter_q0216_01.htm";
				else
					htmltext = "blacksmith_pinter_q0216_02.htm";
			else if(st.ownItemCount(ALLTRANS_INSTRUCTIONS) > 0 && st.ownItemCount(PINTERS_INSTRUCTIONS) > 0)
			...
			else if(st.ownItemCount(ALLTRANS_INSTRUCTIONS) == 1 && st.ownItemCount(PINTERS_INSTRUCTIONS) == 0 && (st.ownItemCount(JOURNEYMAN_DECO_BEADS) > 0 || st.ownItemCount(JOURNEYMAN_RING) > 0))
	...
	public String onKill(NpcInstance npc, QuestState st)
	{
		...
		if(cond == 5 && (npcId == 20079 || npcId == 20080 || npcId == 20081))
			if(Rnd.chance(33) && st.ownItemCount(ALLTRANS_INSTRUCTIONS) > 0 && st.ownItemCount(PINTERS_INSTRUCTIONS) > 0)
			{
				long count = st.ownItemCount(AMBER_BEAD) + st.ownItemCount(AMBER_LUMP) * 5;
				...
				if(count < 70 && st.getPlayer().getClassId().getId() == 0x38)
				{
					...
					if(((MonsterInstance) npc).isSpoiled())
						st.giveItems(AMBER_LUMP, 5);
					count = st.ownItemCount(AMBER_BEAD) + st.ownItemCount(AMBER_LUMP) * 5;
					if(count == 70)
						st.soundEffect(SOUND_MIDDLE);
					else
						st.soundEffect(SOUND_ITEMGET);
