...
public class _508_TheClansReputation extends Quest implements ScriptFile
{
	...
	private static final int[][] REWARDS_LIST = {
			{ 0, 0 },
			{ PALIBATI_QUEEN_THEMIS, THEMIS_SCALE, 85 },
			{ Shilens_Priest_Hisilrome, Hisilromes_Heart, 65 },
			{ GARGOYLE_LORD_TIPHON, TIPHON_SHARD, 50 },
			{ LAST_LESSER_GIANT_GLAKI, GLAKIS_NECLEUS, 125 },
			{ RAHHA, RAHHAS_FANG, 71 },
			{ FLAMESTONE_GIANT, NUCLEUS_OF_FLAMESTONE_GIANT, 80 } };
	...
	public String onKill(NpcInstance npc, QuestState st)
	{
		...
		if(npc.getNpcId() == raid && st.ownItemCount(item) == 0)
		{
			st.giveItems(item, 1);
