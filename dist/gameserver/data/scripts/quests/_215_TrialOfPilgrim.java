...
public class _215_TrialOfPilgrim extends Quest implements ScriptFile
{
	...
	{
		if(st.ownItemCount(MARK_OF_PILGRIM_ID) > 0)
		{
			st.exitQuest(true);
			return "completed";
		}
		...
		else if(npcId == 30648 && st.getCond() == 1 && st.ownItemCount(VOUCHER_OF_TRIAL_ID) > 0)
			htmltext = "hermit_santiago_q0215_09.htm";
		else if(npcId == 30648 && st.getCond() == 17 && st.ownItemCount(BOOK_OF_SAGE_ID) > 0)
		{
		...
		else if(npcId == 30571 && st.getCond() == 1 && st.ownItemCount(VOUCHER_OF_TRIAL_ID) > 0)
		{
		...
		else if(npcId == 30571 && st.getCond() == 5 && st.ownItemCount(SPIRIT_OF_FLAME_ID) > 0)
		...
		else if(npcId == 30649 && st.getCond() == 4 && st.ownItemCount(ESSENSE_OF_FLAME_ID) > 0)
			htmltext = "ancestor_martankus_q0215_03.htm";
		else if(npcId == 30550 && st.getCond() == 5 && st.ownItemCount(SPIRIT_OF_FLAME_ID) > 0)
		{
		...
		else if(npcId == 30650 && st.getCond() == 6 && st.ownItemCount(TAG_OF_RUMOR_ID) > 0)
			htmltext = "gerald_priest_of_earth_q0215_01.htm";
		else if(npcId == 30650 && st.getCond() >= 8 && st.ownItemCount(GREY_BADGE_ID) > 0 && st.ownItemCount(BOOK_OF_GERALD_ID) > 0)
		{
			htmltext = "gerald_priest_of_earth_q0215_04.htm";
			st.giveItems(ADENA_ID, 100000, false);
			st.takeItems(BOOK_OF_GERALD_ID, 1);
		}
		else if(npcId == 30651 && st.getCond() == 6 && st.ownItemCount(TAG_OF_RUMOR_ID) > 0)
		{
		...
		else if(npcId == 30651 && st.getCond() == 7 && st.ownItemCount(TAG_OF_RUMOR_ID) > 0)
		{
		...
		else if(npcId == 30036 && st.getCond() == 12 && st.ownItemCount(STATUE_OF_EINHASAD_ID) > 0)
		...
		else if(npcId == 30362 && st.getCond() == 15 && st.ownItemCount(BOOK_OF_DARKNESS_ID) > 0)
		...
		else if(npcId == 30652 && st.getCond() == 14 && st.ownItemCount(DEBRIS_OF_WILLOW_ID) > 0)
			htmltext = "uruha_q0215_01.htm";
		else if(npcId == 30652 && st.getCond() == 15 && st.ownItemCount(BOOK_OF_DARKNESS_ID) > 0)
		...
		else if(npcId == 30612 && st.getCond() == 16)
		{
			...
			if(st.ownItemCount(BOOK_OF_DARKNESS_ID) > 0)
				st.takeItems(BOOK_OF_DARKNESS_ID, 1);
			if(st.ownItemCount(BOOK_OF_GERALD_ID) > 0)
				st.takeItems(BOOK_OF_GERALD_ID, 1);
			st.setCond(17);
	...
	public String onKill(NpcInstance npc, QuestState st)
	{
		...
		if(npcId == 27116)
		{
			if(st.getCond() == 3 && st.ownItemCount(ESSENSE_OF_FLAME_ID) == 0)
				if(Rnd.chance(30))
				{
		...
		{
			if(st.getCond() == 10 && st.ownItemCount(HAIR_OF_NAHIR_ID) == 0)
			{
			...
			if(st.getCond() == 13 && st.ownItemCount(DEBRIS_OF_WILLOW_ID) == 0)
				if(Rnd.chance(20))
				{
