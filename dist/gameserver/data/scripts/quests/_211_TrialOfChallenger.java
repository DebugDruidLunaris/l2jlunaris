...
public class _211_TrialOfChallenger extends Quest implements ScriptFile
{
	...
	public _211_TrialOfChallenger()
	{
		...
		addQuestItem(new int[] { SCROLL_OF_SHYSLASSY_ID, LETTER_OF_KASH_ID, WATCHERS_EYE1_ID, BROKEN_KEY_ID, WATCHERS_EYE2_ID });
	}
	...
	public String onEvent(String event, QuestState st, NpcInstance npc)
	{
		...
		{
			if(st.ownItemCount(BROKEN_KEY_ID) > 0)
			{
	...
	{
		if(st.ownItemCount(MARK_OF_CHALLENGER_ID) > 0)
		{
			st.exitQuest(true);
			return "completed";
		}
		...
		else if(npcId == Kash && cond == 2 && st.ownItemCount(SCROLL_OF_SHYSLASSY_ID) == 1)
		{
		...
		else if(npcId == Kash && cond == 1 && st.ownItemCount(LETTER_OF_KASH_ID) == 1)
			htmltext = "kash_q0211_08.htm";
		else if(npcId == Kash && cond >= 7)
			htmltext = "kash_q0211_09.htm";
		else if(npcId == Martien && cond == 3 && st.ownItemCount(LETTER_OF_KASH_ID) == 1)
			htmltext = "martian_q0211_01.htm";
		else if(npcId == Martien && cond == 4 && st.ownItemCount(WATCHERS_EYE1_ID) == 0)
			htmltext = "martian_q0211_03.htm";
		else if(npcId == Martien && cond == 5 && st.ownItemCount(WATCHERS_EYE1_ID) > 0)
		{
		...
		else if(npcId == Raldo && cond == 7 && st.ownItemCount(WATCHERS_EYE2_ID) > 0)
	...
	public String onKill(NpcInstance npc, QuestState st)
	{
		int npcId = npc.getNpcId();
		int cond = st.getCond();
		if(npcId == Shyslassys && cond == 1 && st.ownItemCount(SCROLL_OF_SHYSLASSY_ID) == 0 && st.ownItemCount(BROKEN_KEY_ID) == 0)
		{
		...
		else if(npcId == Gorr && cond == 4 && st.ownItemCount(WATCHERS_EYE1_ID) == 0)
		{
		...
		{
			if(st.ownItemCount(WATCHERS_EYE2_ID) == 0)
