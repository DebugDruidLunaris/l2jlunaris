...
public class _212_TrialOfDuty extends Quest implements ScriptFile
{
	...
	{
		if(st.ownItemCount(MARK_OF_DUTY_ID) > 0)
		{
			st.exitQuest(true);
			return "completed";
		}
		...
		{
			if(cond == 18 && st.ownItemCount(LETTER_OF_DUSTIN_ID) > 0)
			{
		...
		else if(npcId == 30653)
		{
			if(cond == 1)
			{
				...
				if(st.ownItemCount(OLD_KNIGHT_SWORD_ID) == 0)
					st.giveItems(OLD_KNIGHT_SWORD_ID, 1);
				st.setCond(2);
			}
			else if(cond == 2 && st.ownItemCount(KNIGHTS_TEAR_ID) == 0)
				htmltext = "sir_aron_tanford_q0212_02.htm";
			else if(cond == 3 && st.ownItemCount(KNIGHTS_TEAR_ID) > 0)
			{
		...
		else if(npcId == 30654)
		{
			...
			else if(cond == 5 && st.ownItemCount(TALIANUSS_REPORT_ID) == 0)
				htmltext = "sir_kiel_nighthawk_q0212_02.htm";
			else if(cond == 6 && st.ownItemCount(TALIANUSS_REPORT_ID) > 0)
			{
				htmltext = "sir_kiel_nighthawk_q0212_03.htm";
				st.setCond(7);
				st.giveItems(MIRROR_OF_ORPIC_ID, 1);
			}
			else if(cond == 6 && st.ownItemCount(MIRROR_OF_ORPIC_ID) > 0)
				htmltext = "sir_kiel_nighthawk_q0212_04.htm";
			else if(st.ownItemCount(TEAR_OF_CONFESSION_ID) > 0)
			{
		...
		else if(npcId == 30656 && cond == 8 && st.ownItemCount(MIRROR_OF_ORPIC_ID) > 0)
		{
		...
		else if(npcId == 30655)
		{
			...
			else if(cond == 12 && st.ownItemCount(MILITAS_ARTICLE_ID) >= 20)
			{
				htmltext = "isael_silvershadow_q0212_04.htm";
				st.takeItems(MILITAS_ARTICLE_ID, st.ownItemCount(MILITAS_ARTICLE_ID));
		...
		{
			if(cond == 13 && st.ownItemCount(TEAR_OF_LOYALTY_ID) > 0)
				htmltext = "dustin_q0212_01.htm";
			else if(cond == 14 && !(st.ownItemCount(ATEBALTS_SKULL_ID) > 0 && st.ownItemCount(ATEBALTS_RIBS_ID) > 0 && st.ownItemCount(ATEBALTS_SHIN_ID) > 0))
			...
			else if(cond == 17 && st.ownItemCount(LETTER_OF_WINDAWOOD_ID) > 0)
			{
			...
			if(cond == 16 && st.ownItemCount(SAINTS_ASHES_URN_ID) > 0)
			{
	...
	public String onKill(NpcInstance npc, QuestState st)
	{
		...
		else if(npcId == 27119 && cond == 2 && st.ownItemCount(OLD_KNIGHT_SWORD_ID) > 0)
		{
			st.giveItems(KNIGHTS_TEAR_ID, 1);
			st.soundEffect(SOUND_MIDDLE);
			st.setCond(3);
		}
		else if((npcId == 20200 || npcId == 20201) && cond == 5 && st.ownItemCount(TALIANUSS_REPORT_ID) == 0)
		{
			if(Rnd.chance(50))
			{
			...
			if(st.ownItemCount(REPORT_PIECE_ID) >= 10)
			{
				st.takeItems(REPORT_PIECE_ID, st.ownItemCount(REPORT_PIECE_ID));
		...
		else if(npcId >= 20577 && npcId <= 20582 && cond == 11 && st.ownItemCount(MILITAS_ARTICLE_ID) < 20)
		{
			if(st.ownItemCount(MILITAS_ARTICLE_ID) == 19)
			{
			...
			if(st.ownItemCount(ATEBALTS_SKULL_ID) == 0)
			{
				st.giveItems(ATEBALTS_SKULL_ID, 1);
				st.soundEffect(SOUND_ITEMGET);
			}
			else if(st.ownItemCount(ATEBALTS_RIBS_ID) == 0)
			{
				st.giveItems(ATEBALTS_RIBS_ID, 1);
				st.soundEffect(SOUND_ITEMGET);
			}
			else if(st.ownItemCount(ATEBALTS_SHIN_ID) == 0)
			{
