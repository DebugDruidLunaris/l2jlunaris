...
public class _10270_BirthOfTheSeed extends Quest implements ScriptFile
{
	...
	public String onTalk(NpcInstance npc, QuestState st)
	{
		...
		else if(npcId == ARTIUS)
		{
			...
			else if(cond == 2 && (st.ownItemCount(Yehan_Klodekus_Badge) == 0 || st.ownItemCount(Yehan_Klanikus_Badge) == 0 || st.ownItemCount(Lich_Crystal) == 0))
				htmltext = "artius_q10270_4.htm";
			else if(cond == 2 && st.ownItemCount(Yehan_Klodekus_Badge) == 1 && st.ownItemCount(Yehan_Klanikus_Badge) == 1 && st.ownItemCount(Lich_Crystal) == 1)
	...
	public String onKill(NpcInstance npc, QuestState st)
	{
		...
		int cond = st.getCond();
		if(cond == 2)
			if(npcId == Yehan_Klodekus && st.ownItemCount(Yehan_Klodekus_Badge) < 1)
			{
				st.giveItems(Yehan_Klodekus_Badge, 1);
				st.soundEffect(SOUND_ITEMGET);
			}
			else if(npcId == Yehan_Klanikus && st.ownItemCount(Yehan_Klanikus_Badge) < 1)
			{
				st.giveItems(Yehan_Klanikus_Badge, 1);
				st.soundEffect(SOUND_ITEMGET);
			}
			else if(npcId == Cohemenes && st.ownItemCount(Lich_Crystal) < 1)
			{
