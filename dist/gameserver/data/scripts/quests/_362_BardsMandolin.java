...
public class _362_BardsMandolin extends Quest implements ScriptFile
{
	...
	public String onTalk(NpcInstance npc, QuestState st)
	{
		...
		if(npcId == SWAN)
		{
			...
			else if(cond == 3 && st.ownItemCount(SWANS_FLUTE) > 0 && st.ownItemCount(SWANS_LETTER) == 0)
			{
				htmltext = "30957_3.htm";
				st.setCond(4);
				st.giveItems(SWANS_LETTER, 1);
			}
			else if(cond == 4 && st.ownItemCount(SWANS_FLUTE) > 0 && st.ownItemCount(SWANS_LETTER) > 0)
		...
		else if(npcId == NANARIN && cond == 4 && st.ownItemCount(SWANS_FLUTE) > 0 && st.ownItemCount(SWANS_LETTER) > 0)
		{
