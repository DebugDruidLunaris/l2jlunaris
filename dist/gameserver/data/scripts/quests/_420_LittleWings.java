...
public class _420_LittleWings extends Quest implements ScriptFile
{
	...
	public String onEvent(String event, QuestState st, NpcInstance npc)
	{
		...
		else if(event.equalsIgnoreCase("30608-03.htm") && _state == STARTED && cond == 2 && st.ownItemCount(Fairy_Stone_List) > 0)
		{
		...
		else if(event.equalsIgnoreCase("30608-03a.htm") && _state == STARTED && cond == 2 && st.ownItemCount(Deluxe_Fairy_Stone_List) > 0)
		{
		...
		else if(event.equalsIgnoreCase("30711-03.htm") && _state == STARTED && cond == 3 && st.ownItemCount(Fairy_Stone) + st.ownItemCount(Deluxe_Fairy_Stone) > 0)
		{
			st.setCond(4);
			st.soundEffect(SOUND_MIDDLE);
			if(st.ownItemCount(Deluxe_Fairy_Stone) > 0)
				return st.getInt("broken") == 1 ? "30711-04a.htm" : "30711-03a.htm";
			if(st.getInt("broken") == 1)
				return "30711-04.htm";
		}
		else if(event.equalsIgnoreCase("30747-02.htm") && _state == STARTED && cond == 4 && st.ownItemCount(Fairy_Stone) > 0)
		{
			st.takeItems(Fairy_Stone, -1);
			st.setMemoState("takedStone", "1");
		}
		else if(event.equalsIgnoreCase("30747-02a.htm") && _state == STARTED && cond == 4 && st.ownItemCount(Deluxe_Fairy_Stone) > 0)
		{
			st.takeItems(Deluxe_Fairy_Stone, -1);
			st.setMemoState("takedStone", "2");
		...
		{
			st.setCond(5);
			st.removeMemo("takedStone");
			st.giveItems(Juice_of_Monkshood, 1);
			st.soundEffect(SOUND_ITEMGET);
		}

		else if(event.equalsIgnoreCase("30748-02.htm") && cond == 5 && _state == STARTED && st.ownItemCount(Juice_of_Monkshood) > 0)
		{
			st.setCond(6);
		...
		else if(event.equalsIgnoreCase("30749-02.htm") && cond == 5 && _state == STARTED && st.ownItemCount(Juice_of_Monkshood) > 0)
		{
			st.setCond(6);
			st.takeItems(Juice_of_Monkshood, -1);
			st.giveItems(3824, 1);
		...
		else if(event.equalsIgnoreCase("30750-02.htm") && cond == 5 && _state == STARTED && st.ownItemCount(Juice_of_Monkshood) > 0)
		{
			st.setCond(6);
			st.takeItems(Juice_of_Monkshood, -1);
			st.giveItems(3826, 1);
		...
		else if(event.equalsIgnoreCase("30751-02.htm") && cond == 5 && _state == STARTED && st.ownItemCount(Juice_of_Monkshood) > 0)
		{
			st.setCond(6);
			st.takeItems(Juice_of_Monkshood, -1);
			st.giveItems(3828, 1);
		...
		else if(event.equalsIgnoreCase("30752-02.htm") && cond == 5 && _state == STARTED && st.ownItemCount(Juice_of_Monkshood) > 0)
		{
			st.setCond(6);
			st.takeItems(Juice_of_Monkshood, -1);
			st.giveItems(3830, 1);
		...
		else if(event.equalsIgnoreCase("30747-09.htm") && _state == STARTED && cond == 7)
		{
			int egg_id = 0;
			for(int[] wyrm : wyrms)
				if(st.ownItemCount(wyrm[2]) == 0 && st.ownItemCount(wyrm[3]) >= 1)
				{
					egg_id = wyrm[3];
					break;
				}
			...
			if(st.ownItemCount(Fairy_Dust) > 0)
			{
				st.soundEffect(SOUND_MIDDLE);
	...
	public String onTalk(NpcInstance npc, QuestState st)
	{
		...
		if(npcId == Byron)
		{
			...
			if(cond == 2 && broken == 1)
				return "30711-07.htm";
			if(cond == 3 && st.ownItemCount(Fairy_Stone) + st.ownItemCount(Deluxe_Fairy_Stone) > 0)
				return "30711-01.htm";
			if(cond >= 4 && st.ownItemCount(Deluxe_Fairy_Stone) > 0)
				return "30711-05a.htm";
			if(cond >= 4 && st.ownItemCount(Fairy_Stone) > 0)
				return "30711-05.htm";
		}
		...
		if(npcId == Mimyu)
		{
			...
			{
				for(int[] wyrm : wyrms)
					if(st.ownItemCount(wyrm[2]) == 0 && st.ownItemCount(wyrm[3]) >= 20)
						return "30747-07.htm";
				return "30747-06.htm";
			}
			if(cond == 7)
		...
		{
			if(cond == 5 && st.ownItemCount(Juice_of_Monkshood) > 0)
				return String.valueOf(npcId) + "-01.htm";
			if(cond == 6 && st.ownItemCount(getWyrmScale(npcId)) > 0)
			{
				int egg_id = getWyrmEgg(npcId);
				if(st.ownItemCount(egg_id) < 20)
			...
			if(cond == 7 && st.ownItemCount(getWyrmEgg(npcId)) == 1)
	...
	public String onKill(NpcInstance npc, QuestState st)
	{
		...
		if(cond == 2 && npcId == Toad_Lord)
		{
			...
			if(st.ownItemCount(Toad_Lord_Back_Skin) < needed_skins && Rnd.chance(Toad_Lord_Back_Skin_Chance))
			{
				st.giveItems(Toad_Lord_Back_Skin, 1);
				st.soundEffect(st.ownItemCount(Toad_Lord_Back_Skin) < needed_skins ? SOUND_ITEMGET : SOUND_MIDDLE);
			}
			return null;
		}

		if(npcId >= Enchanted_Valey_First && npcId <= Enchanted_Valey_Last && st.ownItemCount(Deluxe_Fairy_Stone) > 0)
		{
			st.takeItems(Deluxe_Fairy_Stone, 1);
			st.setMemoState("broken", "1");
			st.setCond(1);
		...
		if(cond == 6)
		{
			...
			if(wyrm_id > 0 && st.ownItemCount(getWyrmScale(wyrm_id)) > 0 && st.ownItemCount(getWyrmEgg(wyrm_id)) < 20 && Rnd.chance(Egg_Chance))
			{
				st.giveItems(getWyrmEgg(wyrm_id), 1);
				st.soundEffect(st.ownItemCount(getWyrmEgg(wyrm_id)) < 20 ? SOUND_ITEMGET : SOUND_MIDDLE);
			}
		}
	...
	public static boolean CheckFairyStoneItems(QuestState st, int[][] item_list)
	{
		for(int[] _item : item_list)
			if(st.ownItemCount(_item[0]) < _item[1])
				return false;
		return true;
	}
