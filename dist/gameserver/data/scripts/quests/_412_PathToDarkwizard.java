...
public class _412_PathToDarkwizard extends Quest implements ScriptFile
{
	...
	public String onEvent(String event, QuestState st, NpcInstance npc)
	{
		...
		{
			if(st.getPlayer().getLevel() >= 18 && st.getPlayer().getClassId().getId() == 0x26 && st.ownItemCount(JEWEL_OF_DARKNESS_ID) < 1)
			{
				st.setCond(1);
				st.setState(STARTED);
			...
			else if(st.getPlayer().getLevel() >= 18 && st.getPlayer().getClassId().getId() == 0x26 && st.ownItemCount(JEWEL_OF_DARKNESS_ID) > 0)
				htmltext = "varika_q0412_04.htm";
		}
		else if(event.equalsIgnoreCase("412_1"))
		{
			if(st.ownItemCount(SEEDS_OF_ANGER_ID) > 0)
		...
		{
			if(st.ownItemCount(SEEDS_OF_HORROR_ID) > 0)
		...
		{
			if(st.ownItemCount(SEEDS_OF_LUNACY_ID) > 0)
				htmltext = "varika_q0412_12.htm";
			else if(st.ownItemCount(SEEDS_OF_LUNACY_ID) < 1 && st.ownItemCount(SEEDS_OF_DESPAIR_ID) > 0)
	...
	public String onTalk(NpcInstance npc, QuestState st)
	{
		...
		if(npcId == VARIKA)
		{
			if(cond < 1)
			{
				if(st.ownItemCount(JEWEL_OF_DARKNESS_ID) < 1)
					htmltext = "varika_q0412_01.htm";
				else
					htmltext = "varika_q0412_04.htm";
			}
			else if(st.ownItemCount(SEEDS_OF_DESPAIR_ID) > 0 && st.ownItemCount(SEEDS_OF_HORROR_ID) > 0 && st.ownItemCount(SEEDS_OF_LUNACY_ID) > 0 && st.ownItemCount(SEEDS_OF_ANGER_ID) > 0)
			{
			...
			else if(st.ownItemCount(SEEDS_OF_DESPAIR_ID) > 0)
				if(st.ownItemCount(FAMILYS_ASHES_ID) < 1 && st.ownItemCount(LUCKY_KEY_ID) < 1 && st.ownItemCount(CANDLE_ID) < 1 && st.ownItemCount(HUB_SCENT_ID) < 1 && st.ownItemCount(KNEE_BONE_ID) < 1 && st.ownItemCount(HEART_OF_LUNACY_ID) < 1)
					htmltext = "varika_q0412_17.htm";
				else if(st.ownItemCount(SEEDS_OF_ANGER_ID) < 1)
					htmltext = "varika_q0412_08.htm";
				else if(st.ownItemCount(SEEDS_OF_HORROR_ID) > 0)
					htmltext = "varika_q0412_19.htm";
				else if(st.ownItemCount(HEART_OF_LUNACY_ID) < 1)
					htmltext = "varika_q0412_13.htm";
		}
		else if(npcId == ARKENIA && cond > 0 && st.ownItemCount(SEEDS_OF_LUNACY_ID) < 1)
		{
			if(st.ownItemCount(HUB_SCENT_ID) < 1 && st.ownItemCount(HEART_OF_LUNACY_ID) < 1)
			{
				htmltext = "arkenia_q0412_01.htm";
				st.giveItems(HUB_SCENT_ID, 1);
			}
			else if(st.ownItemCount(HUB_SCENT_ID) > 0 && st.ownItemCount(HEART_OF_LUNACY_ID) < 3)
		...
		{
			if(st.ownItemCount(SEEDS_OF_ANGER_ID) < 1)
			{
				if(st.ownItemCount(SEEDS_OF_DESPAIR_ID) > 0 && st.ownItemCount(FAMILYS_ASHES_ID) < 1 && st.ownItemCount(LUCKY_KEY_ID) < 1)
					htmltext = "charkeren_q0412_01.htm";
				else if(st.ownItemCount(SEEDS_OF_DESPAIR_ID) > 0 && st.ownItemCount(FAMILYS_ASHES_ID) < 3 && st.ownItemCount(LUCKY_KEY_ID) > 0)
		...
		else if(npcId == ANNIKA && cond > 0 && st.ownItemCount(SEEDS_OF_HORROR_ID) < 1)
			if(st.ownItemCount(SEEDS_OF_DESPAIR_ID) > 0 && st.ownItemCount(CANDLE_ID) < 1 && st.ownItemCount(KNEE_BONE_ID) < 1)
				htmltext = "annsery_q0412_01.htm";
			else if(st.ownItemCount(SEEDS_OF_DESPAIR_ID) > 0 && st.ownItemCount(CANDLE_ID) > 0 && st.ownItemCount(KNEE_BONE_ID) < 2)
	...
	public String onKill(NpcInstance npc, QuestState st)
	{
		//DROPLIST [MOB_ID, REQUIRED, ITEM, NEED_COUNT]
		for(int[] element : DROPLIST)
			if(st.getCond() == 1 && npc.getNpcId() == element[0] && st.ownItemCount(element[1]) > 0)
				if(Rnd.chance(50) && st.ownItemCount(element[2]) < element[3])
				{
					st.giveItems(element[2], 1);
					if(st.ownItemCount(element[2]) == element[3])
