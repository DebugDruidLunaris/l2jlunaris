...
public class _066_CertifiedArbalester extends Quest implements ScriptFile
{
	...
	public String onTalk(NpcInstance npc, QuestState st)
	{
		...
		if(npcId == Rindy)
		{
			if(st.ownItemCount(KamaelInquisitorMark) > 0)
			{
		...
		else if(npcId == Clayton)
		{
			...
			{
				if(st.getInt("id") == 0 && st.ownItemCount(EnmityCrystal) == 30)
				{
				...
				else if(st.getInt("id") == 0 && st.ownItemCount(EnmityCrystal) < 30)
				{
					htmltext = "30464-06.htm";
					st.setCond(2);
				}
		...
		else if(npcId == Holvas)
		{
			...
			{
				if(st.ownItemCount(ManuscriptPage) == 30)
		...
		else if(npcId == Gaius)
		{
			if(cond == 12)
				...
				if(st.ownItemCount(ManashenTalisman) == 10)
