...
public class _229_TestOfWitchcraft extends Quest implements ScriptFile
{
	...
	public String onEvent(String event, QuestState st, NpcInstance npc)
	{
		...
		else if(event.equalsIgnoreCase("30630-14.htm"))
		{
			...
			else
			{
				...
				if(st.ownItemCount(Brimstone_1st) == 0)
		...
		else if(event.equalsIgnoreCase("30110-08.htm"))
		{
			...
			if(st.ownItemCount(SwordOfBinding) > 0)
			{
				st.setCond(7);
				st.setState(STARTED);
			}
		...
		else if(event.equalsIgnoreCase("30633-02.htm"))
		{
			...
			{
				st.setMemoState("id", "2");
				st.setCond(9);
				if(st.ownItemCount(Brimstone_2nd) == 0)
	...
	public String onTalk(NpcInstance npc, QuestState st)
	{
		...
		if(npcId == Orim)
		{
			if(st.ownItemCount(MarkOfWitchcraft) != 0)
			{
				htmltext = "completed";
				st.exitQuest(true);
			}
			...
			else if(cond == 7)
			{
				...
				if(st.ownItemCount(ZeruelBindCrystal) != 0)
					htmltext = "30630-19.htm";
				else if(st.ownItemCount(PurgatoryKey) != 0)
		...
		else if(npcId == Iker)
		{
			if(cond == 2)
			{
				if(st.ownItemCount(Aklantoth_1stGem) == 0 && st.ownItemCount(IkersList) == 0)
					htmltext = "30110-01.htm";
				else if(st.ownItemCount(IkersList) > 0 && (st.ownItemCount(DireWyrmFang) < 20 || st.ownItemCount(LetoLizardmanCharm) < 20 || st.ownItemCount(EnchantedGolemHeartstone) < 20))
					htmltext = "30110-04.htm";
				else if(st.ownItemCount(Aklantoth_1stGem) == 0 && st.ownItemCount(IkersList) > 0)
				{
				...
				else if(st.ownItemCount(Aklantoth_1stGem) == 1)
		...
		{
			if(cond == 2)
				if(st.ownItemCount(Aklantoth_2stGem) == 0)
		...
		else if(npcId == Lara)
		{
			if(cond == 2)
			{
				if(st.ownItemCount(LarasMemo) == 0 && st.ownItemCount(Aklantoth_3stGem) == 0)
					htmltext = "30063-01.htm";
				else if(st.ownItemCount(LarasMemo) == 1 && st.ownItemCount(Aklantoth_3stGem) == 0)
					htmltext = "30063-03.htm";
				else if(st.ownItemCount(Aklantoth_3stGem) == 1)
					htmltext = "30063-04.htm";
			}
			else if(cond > 2)
				htmltext = "30063-05.htm";
		}
		else if(npcId == Roderik && cond == 2 && st.ownItemCount(LarasMemo) > 0)
			htmltext = "30631-01.htm";
		else if(npcId == Nestle && cond == 2)
			if(st.ownItemCount(Aklantoth_1stGem) > 0 && st.ownItemCount(Aklantoth_2stGem) > 0 && st.ownItemCount(Aklantoth_3stGem) > 0)
				htmltext = "30314-01.htm";
			else
				htmltext = "30314-04.htm";
		else if(npcId == Leopold)
		{
			if(cond == 2 && st.ownItemCount(NestlesMemo) > 0)
			{
				if(st.ownItemCount(Aklantoth_4stGem) + st.ownItemCount(Aklantoth_5stGem) + st.ownItemCount(Aklantoth_6stGem) == 0)
		...
		else if(npcId == Vasper)
		{
			if(cond == 6)
			{
				if(st.ownItemCount(SirVaspersLetter) > 0 || st.ownItemCount(VadinsCrucifix) > 0)
					htmltext = "30417-04.htm";
				else if(st.ownItemCount(VadinsSanctions) == 0)
				...
				else if(st.ownItemCount(VadinsSanctions) != 0)
				{
					...
					if(st.ownItemCount(SoultrapCrystal) > 0)
					{
						st.setCond(7);
						st.setState(STARTED);
					}
		...
		else if(npcId == Vadin)
		{
			if(cond == 6)
			{
				if(st.ownItemCount(SirVaspersLetter) != 0)
				{
					htmltext = "30188-01.htm";
					st.takeItems(SirVaspersLetter, 1);
					st.giveItems(VadinsCrucifix, 1);
				}
				else if(st.ownItemCount(VadinsCrucifix) > 0 && st.ownItemCount(TamlinOrcAmulet) < 20)
					htmltext = "30188-02.htm";
				else if(st.ownItemCount(TamlinOrcAmulet) >= 20)
				{
		...
		{
			if(st.getInt("id") == 2 || cond == 8 && st.ownItemCount(Brimstone_2nd) == 0)
	...
	public String onKill(NpcInstance npc, QuestState st)
	{
		...
		if(cond == 2 && st.ownItemCount(LeopoldsJournal) > 0 && npcId == SkeletalMercenary)
		{
			if(st.ownItemCount(Aklantoth_4stGem) == 0 && Rnd.chance(50))
				st.giveItems(Aklantoth_4stGem, 1);
			if(st.ownItemCount(Aklantoth_5stGem) == 0 && Rnd.chance(50))
				st.giveItems(Aklantoth_5stGem, 1);
			if(st.ownItemCount(Aklantoth_6stGem) == 0 && Rnd.chance(50))
				st.giveItems(Aklantoth_6stGem, 1);
			if(st.ownItemCount(Aklantoth_4stGem) != 0 && st.ownItemCount(Aklantoth_5stGem) != 0 && st.ownItemCount(Aklantoth_6stGem) != 0)
			{
