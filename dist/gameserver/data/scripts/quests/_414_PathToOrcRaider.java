...
public class _414_PathToOrcRaider extends Quest implements ScriptFile
{
	...
	public String onTalk(NpcInstance npc, QuestState st)
	{
		...
		if(npcId == KARUKIA)
		{
			if(cond < 1)
			{
				if(playerLvl >= 18 && playerClassID == 0x2c && st.ownItemCount(MARK_OF_RAIDER) == 0 && st.ownItemCount(GOBLIN_DWELLING_MAP) == 0)
				...
				else if(playerLvl >= 18 && playerClassID == 0x2c && st.ownItemCount(MARK_OF_RAIDER) > 0)
					htmltext = "prefect_karukia_q0414_04.htm";
				else
					htmltext = "prefect_karukia_q0414_02.htm";
			}
			else if(cond == 1 && st.ownItemCount(GOBLIN_DWELLING_MAP) > 0 && st.ownItemCount(KURUKA_RATMAN_TOOTH) < 10)
				htmltext = "prefect_karukia_q0414_06.htm";
			else if(cond == 2 && st.ownItemCount(GOBLIN_DWELLING_MAP) > 0 && st.ownItemCount(KURUKA_RATMAN_TOOTH) > 9)
				htmltext = "prefect_karukia_q0414_07.htm";
			else if(cond == 3 && st.ownItemCount(BETRAYER_UMBAR_REPORT) > 0 && st.ownItemCount(HEAD_OF_BETRAYER) < 2)
				htmltext = "prefect_karukia_q0414_08.htm";
			else if(cond == 4 && st.ownItemCount(BETRAYER_UMBAR_REPORT) > 0 && st.ownItemCount(HEAD_OF_BETRAYER) == 2)
				htmltext = "prefect_karukia_q0414_09.htm";
		}
		else if(npcId == KASMAN && cond > 0)
		{
			if(cond == 3 && st.ownItemCount(BETRAYER_UMBAR_REPORT) > 0 && st.ownItemCount(HEAD_OF_BETRAYER) < 1)
				htmltext = "prefect_kasman_q0414_01.htm";
			else if(cond == 3 && st.ownItemCount(HEAD_OF_BETRAYER) > 0 && st.ownItemCount(HEAD_OF_BETRAYER) < 2)
				htmltext = "prefect_kasman_q0414_02.htm";
			else if(cond == 4 && st.ownItemCount(HEAD_OF_BETRAYER) > 1)
			{
			...
			else if(cond == 6 && st.ownItemCount(TIMORA_ORCS_HEAD) < 1)
				htmltext = "prefect_tazar_q0414_03.htm";
			else if(cond == 7 && st.ownItemCount(TIMORA_ORCS_HEAD) > 0)
			{
	...
	public String onKill(NpcInstance npc, QuestState st)
	{
		...
		{
			if(st.ownItemCount(GOBLIN_DWELLING_MAP) == 1 && st.ownItemCount(KURUKA_RATMAN_TOOTH) < 10 && st.ownItemCount(GREEN_BLOOD) < 40)
				if(st.ownItemCount(GREEN_BLOOD) > 20 && Rnd.chance((st.ownItemCount(GREEN_BLOOD) - 20) * 5))
				{
		...
		{
			if(st.ownItemCount(GOBLIN_DWELLING_MAP) > 0 && st.ownItemCount(KURUKA_RATMAN_TOOTH) < 10)
			{
				st.giveItems(KURUKA_RATMAN_TOOTH, 1);
				if(st.ownItemCount(KURUKA_RATMAN_TOOTH) > 9)
				{
					st.setCond(2);
					st.soundEffect(SOUND_MIDDLE);
				}
		...
		{
			if(st.ownItemCount(BETRAYER_UMBAR_REPORT) > 0 && st.ownItemCount(HEAD_OF_BETRAYER) < 2)
			{
				st.giveItems(HEAD_OF_BETRAYER, 1);
				if(st.ownItemCount(HEAD_OF_BETRAYER) > 1)
				{
					st.setCond(4);
			...
			if(st.ownItemCount(TIMORA_ORCS_HEAD) < 1 && Rnd.chance(50))
			{
