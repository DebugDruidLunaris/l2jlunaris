...
public class _025_HidingBehindTheTruth extends Quest implements ScriptFile
{
	...
	public String onEvent(String event, QuestState st, NpcInstance npc)
	{
		...
		else if(event.equalsIgnoreCase("reply_9"))
		{
			if(GetMemoState == 8 && st.ownItemCount(q_triols_totem3) == 0)
				...
				{
					st.setCond(7);
					st.setMemoState("spawned_triyol_zzolda", String.valueOf(1), true);
					st.setMemoState("triyol_zzolda_player_name", st.getPlayer().getName(), true);
				...
				else if(triyol_zzolda_player_name == st.getPlayer().getName())
		...
		{
			st.removeMemo("spawned_triyol_zzolda");
			if(npc != null)
				npc.deleteMe();
			return null;
		}
		...
		else if(event.equalsIgnoreCase("reply_1a"))
		{
			...
			{
				st.setCond(12);
				NpcInstance box1 = st.addSpawn(q_forest_box1, 60104, -35820, -681);
				st.startQuestTimer("2503", 20000, box1);
				st.soundEffect(SOUND_MIDDLE);
				htmltext = "q_forest_stone2_q0025_02.htm";
			}
		}
		else if(event.equalsIgnoreCase("2503"))
		{
			if(npc != null)
				npc.deleteMe();
			return null;
		}
		else if(event.equalsIgnoreCase("2504"))
		{
			if(npc != null)
				npc.deleteMe();
			return null;
	...
	public String onAttack(NpcInstance npc, QuestState st)
	{
		...
		if(npcId == triyol_zzolda && triyol_zzolda_hp_precent <= 30)
			if(GetMemoState == 8 && st.ownItemCount(q_triols_totem3) == 0)
				if(triyol_zzolda_player_name == st.getPlayer().getName())
				{
					st.setCond(8);
					...
					st.removeMemo("spawned_triyol_zzolda");
					st.removeMemo("triyol_zzolda_player_name");
