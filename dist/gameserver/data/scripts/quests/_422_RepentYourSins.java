...
public class _422_RepentYourSins extends Quest implements ScriptFile
{
	...
	public String onEvent(String event, QuestState st, NpcInstance npc)
	{
		...
		{
			if(st.ownItemCount(PENITENTS_MANACLES1) >= 1)
				st.takeItems(PENITENTS_MANACLES1, -1);
			if(st.ownItemCount(PENITENTS_MANACLES) >= 1)
				st.takeItems(PENITENTS_MANACLES, -1);
			st.setCond(16);
			st.setMemoState("level", String.valueOf(st.getPlayer().getLevel()));
		...
		if(event.equalsIgnoreCase("3"))
		{
			...
			else
			{
				...
				if(st.getPlayer().getPkKills() <= Pk_remove)
				{
					...
					if(st.ownItemCount(PENITENTS_MANACLES2) < 1)
	...
	public String onTalk(NpcInstance npc, QuestState st)
	{
		...
		if(npcId == Black_Judge)
		{
			...
			if(cond <= 13 && cond > 9 && st.ownItemCount(MANUAL_OF_MANACLES) < 1)
			{
				if(st.ownItemCount(PENITENTS_MANACLES2) < 1)
				{
					st.setCond(14);
			...
			if(cond == 14 && st.ownItemCount(MANUAL_OF_MANACLES) > 0)
				return "black_judge_q0422_09.htm";
			if(cond == 15 && st.ownItemCount(PENITENTS_MANACLES1) > 0)
				return "black_judge_q0422_10.htm";
			if(cond >= 16)
			{
				if(st.ownItemCount(PENITENTS_MANACLES) > 0)
				{
					int plevel = findPetLvl(st);
					int level = st.getPlayer().getLevel();
		...
		if(npcId == Katari)
		{
			...
			if(cond == 6 && st.ownItemCount(SCAVENGER_WERERAT_SKULL) < 10)
		...
		if(npcId == Piotur)
		{
			...
			if(cond == 7 && st.ownItemCount(TUREK_WARHOUND_TAIL) < 10)
		...
		if(npcId == Casian)
		{
			...
			if(cond == 8)
			{
				if(st.ownItemCount(TYRANT_KINGPIN_HEART) < 1)
		...
		if(npcId == Joan)
		{
			...
			if(cond == 9 && st.ownItemCount(TRISALIM_TARANTULAS_VENOM_SAC) < 3)
				return "magister_joan_q0422_02.htm";
			if(st.ownItemCount(TRISALIM_TARANTULAS_VENOM_SAC) >= 3)
			{
				st.setCond(13);
		...
		if(npcId == Pushkin)
			if(cond >= 14)
			{
				if(st.ownItemCount(MANUAL_OF_MANACLES) == 1)
				{
					if(st.ownItemCount(SILVER_NUGGET) < 10 || st.ownItemCount(STEEL) < 5 || st.ownItemCount(ADAMANTINE_NUGGET) < 2 || st.ownItemCount(COKES) < 10 || st.ownItemCount(BLACKSMITHS_FRAME) < 1)
				...
				if(st.ownItemCount(PENITENTS_MANACLES1) > 0 || st.ownItemCount(PENITENTS_MANACLES) > 0 || st.ownItemCount(PENITENTS_MANACLES2) > 0)
	...
	public String onKill(NpcInstance npc, QuestState st)
	{
		...
		int cond = st.getCond();
		int npcId = npc.getNpcId();
		long skulls = st.ownItemCount(SCAVENGER_WERERAT_SKULL);
		long tails = st.ownItemCount(TUREK_WARHOUND_TAIL);
		long heart = st.ownItemCount(TYRANT_KINGPIN_HEART);
		long sacs = st.ownItemCount(TRISALIM_TARANTULAS_VENOM_SAC);
