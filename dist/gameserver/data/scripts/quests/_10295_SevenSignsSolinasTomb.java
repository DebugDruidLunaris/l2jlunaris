...
public class _10295_SevenSignsSolinasTomb extends Quest implements ScriptFile
{
	...
	public String onEvent(String event, QuestState st, NpcInstance npc)
	{
		...
		{
			if(st.ownItemCount(StaffofBlessing) > 0)
			{
		...
		{
			if(st.ownItemCount(ScrollofAbstinence) > 0)
			{
		...
		{
			if(st.ownItemCount(SwordofHolySpirit) > 0)
			{
		...
		{
			if(st.ownItemCount(ShieldofSacrifice) > 0)
			{
