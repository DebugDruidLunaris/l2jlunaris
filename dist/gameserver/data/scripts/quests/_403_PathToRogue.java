...
public class _403_PathToRogue extends Quest implements ScriptFile
{
	...
	public _403_PathToRogue()
	{
		...
		addQuestItem(new int[] { NETIS_BOW_ID, NETIS_DAGGER_ID, WANTED_BILL_ID, HORSESHOE_OF_LIGHT_ID, BEZIQUES_LETTER_ID, SPATOIS_BONES_ID });
	}
	...
	public String onEvent(String event, QuestState st, NpcInstance npc)
	{
		...
		if(event.equalsIgnoreCase("30379_2"))
		{
			if(st.getPlayer().getClassId().getId() == 0x00)
			{
				...
				{
					if(st.ownItemCount(BEZIQUES_RECOMMENDATION_ID) > 0)
		...
		else if(event.equalsIgnoreCase("30425_1"))
		{
			...
			if(st.ownItemCount(NETIS_BOW_ID) < 1)
				st.giveItems(NETIS_BOW_ID, 1);
			if(st.ownItemCount(NETIS_DAGGER_ID) < 1)
	...
	public String onTalk(NpcInstance npc, QuestState st)
	{
		...
		if(npcId == BEZIQUE)
		{
			if(cond == 6 && st.ownItemCount(HORSESHOE_OF_LIGHT_ID) < 1 && st.ownItemCount(STOLEN_JEWELRY_ID) + st.ownItemCount(STOLEN_TOMES_ID) + st.ownItemCount(STOLEN_RING_ID) + st.ownItemCount(STOLEN_NECKLACE_ID) == 4)
			{
			...
			else if(cond == 1 && st.ownItemCount(HORSESHOE_OF_LIGHT_ID) < 1 && st.ownItemCount(BEZIQUES_LETTER_ID) > 0)
				htmltext = "captain_bezique_q0403_07.htm";
			else if(cond == 4 && st.ownItemCount(HORSESHOE_OF_LIGHT_ID) > 0)
			{
			...
			else if(cond > 1 && st.ownItemCount(NETIS_BOW_ID) > 0 && st.ownItemCount(NETIS_DAGGER_ID) > 0 && st.ownItemCount(WANTED_BILL_ID) < 1)
				htmltext = "captain_bezique_q0403_10.htm";
			else if(cond == 5 && st.ownItemCount(WANTED_BILL_ID) > 0)
				htmltext = "captain_bezique_q0403_11.htm";
			else
				htmltext = "captain_bezique_q0403_01.htm";
		}
		else if(npcId == NETI)
			if(cond == 1 && st.ownItemCount(BEZIQUES_LETTER_ID) > 0)
				htmltext = "neti_q0403_01.htm";
			else if(cond == 2 | cond == 3 && st.ownItemCount(SPATOIS_BONES_ID) < 10)
			{
				htmltext = "neti_q0403_06.htm";
				st.setCond(2);
			}
			else if(cond == 3 && st.ownItemCount(SPATOIS_BONES_ID) > 9)
			{
	...
	public String onKill(NpcInstance npc, QuestState st)
	{
		...
		if(netis_cond == 1 && st.getItemEquipped(Inventory.PAPERDOLL_RHAND) == NETIS_BOW_ID || st.getItemEquipped(Inventory.PAPERDOLL_RHAND) == NETIS_DAGGER_ID)
		{
			...
			switch(cond)
			{
				case 2:
					for(int[] element : MobsTable)
						if(npcId == element[0] && Rnd.chance(10 * element[1]) && st.ownItemCount(SPATOIS_BONES_ID) < 10)
						{
							st.giveItems(SPATOIS_BONES_ID, 1);
							if(st.ownItemCount(SPATOIS_BONES_ID) == 10)
							{
				...
				case 5:
					if(npcId == CATS_EYE_BANDIT)
						if(st.ownItemCount(WANTED_BILL_ID) > 0)
						{
							int n = Rnd.get(4);
							if(st.ownItemCount(STOLEN_ITEM[n]) == 0)
							{
								st.giveItems(STOLEN_ITEM[n], 1);
								if(st.ownItemCount(STOLEN_JEWELRY_ID) + st.ownItemCount(STOLEN_TOMES_ID) + st.ownItemCount(STOLEN_RING_ID) + st.ownItemCount(STOLEN_NECKLACE_ID) < 4)
									st.soundEffect(SOUND_ITEMGET);
								else
								{
									st.soundEffect(SOUND_MIDDLE);
	...
	public String onAttack(NpcInstance npc, QuestState st)
	{
		...
		if(st.getItemEquipped(Inventory.PAPERDOLL_RHAND) != NETIS_BOW_ID && st.getItemEquipped(Inventory.PAPERDOLL_RHAND) != NETIS_DAGGER_ID)
			st.setMemoState("netis_cond", "0");
		else if(netis_cond == 0)
		{
			st.setMemoState("netis_cond", "1");
