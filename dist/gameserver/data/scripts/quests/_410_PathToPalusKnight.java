...
public class _410_PathToPalusKnight extends Quest implements ScriptFile
{
	...
	public String onEvent(String event, QuestState st, NpcInstance npc)
	{
		...
		{
			if(st.getPlayer().getLevel() >= 18 && st.getPlayer().getClassId().getId() == 0x1f && st.ownItemCount(GAZE_OF_ABYSS_ID) == 0)
			...
			else if(st.getPlayer().getLevel() >= 18 && st.getPlayer().getClassId().getId() == 0x1f && st.ownItemCount(GAZE_OF_ABYSS_ID) == 1)
	...
	public String onTalk(NpcInstance npc, QuestState st)
	{
		...
		if(npcId == VIRGIL)
		{
			...
			else if(st.ownItemCount(PALLUS_TALISMAN_ID) > 0)
			{
				if(st.ownItemCount(LYCANTHROPE_SKULL_ID) < 1)
					htmltext = "master_virgil_q0410_07.htm";
				else if(st.ownItemCount(LYCANTHROPE_SKULL_ID) > 0 && st.ownItemCount(LYCANTHROPE_SKULL_ID) < 13)
					htmltext = "master_virgil_q0410_08.htm";
				else if(st.ownItemCount(LYCANTHROPE_SKULL_ID) > 12)
					htmltext = "master_virgil_q0410_09.htm";
			}
			else if(st.ownItemCount(COFFIN_ETERNAL_REST_ID) > 0)
			{
			...
			else if(st.ownItemCount(MORTE_TALISMAN_ID) > 0 | st.ownItemCount(VIRGILS_LETTER_ID) > 0)
				htmltext = "master_virgil_q0410_12.htm";
		}
		else if(npcId == KALINTA && cond > 0)
			if(st.ownItemCount(VIRGILS_LETTER_ID) > 0)
				htmltext = "kalinta_q0410_01.htm";
			else if(st.ownItemCount(MORTE_TALISMAN_ID) > 0)
				if(st.ownItemCount(TRIMDEN_SILK_ID) < 1 && st.ownItemCount(PREDATOR_CARAPACE_ID) < 1)
					htmltext = "kalinta_q0410_03.htm";
				else if(st.ownItemCount(TRIMDEN_SILK_ID) < 1 | st.ownItemCount(PREDATOR_CARAPACE_ID) < 1)
					htmltext = "kalinta_q0410_04.htm";
				else if(st.ownItemCount(TRIMDEN_SILK_ID) > 4 && st.ownItemCount(PREDATOR_CARAPACE_ID) > 0)
	...
	public String onKill(NpcInstance npc, QuestState st)
	{
		...
		{
			if(cond == 1 && st.ownItemCount(PALLUS_TALISMAN_ID) > 0 && st.ownItemCount(LYCANTHROPE_SKULL_ID) < 13)
			{
				st.giveItems(LYCANTHROPE_SKULL_ID, 1);
				if(st.ownItemCount(LYCANTHROPE_SKULL_ID) > 12)
				{
		...
		{
			if(cond == 4 && st.ownItemCount(MORTE_TALISMAN_ID) > 0 && st.ownItemCount(PREDATOR_CARAPACE_ID) < 1)
			{
				st.giveItems(PREDATOR_CARAPACE_ID, 1);
				st.soundEffect(SOUND_MIDDLE);
				if(st.ownItemCount(TRIMDEN_SILK_ID) > 4)
					st.setCond(5);
			}
		}
		else if(npcId == ARACHNID_TRACKER)
			if(cond == 4 && st.ownItemCount(MORTE_TALISMAN_ID) > 0 && st.ownItemCount(TRIMDEN_SILK_ID) < 5)
			{
				st.giveItems(TRIMDEN_SILK_ID, 1);
				if(st.ownItemCount(TRIMDEN_SILK_ID) > 4)
				{
					st.soundEffect(SOUND_MIDDLE);
					if(st.ownItemCount(PREDATOR_CARAPACE_ID) > 0)
						st.setCond(5);
				}
				else
					st.soundEffect(SOUND_ITEMGET);
