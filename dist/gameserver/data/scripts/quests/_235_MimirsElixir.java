		...
		switch(id)
		{
			case CREATED:
				if(npcId == magister_ladd)
					...
					{
						if(st.getPlayer().getLevel() >= 75 && st.ownItemCount(q_star_of_destiny) >= 1)
							htmltext = "magister_ladd_q0235_01.htm";
						else if(st.getPlayer().getLevel() >= 75 && st.ownItemCount(q_star_of_destiny) < 1)
							htmltext = "magister_ladd_q0235_01b.htm";
						else if(st.getPlayer().getLevel() < 75)
