...
public class _402_PathToKnight extends Quest implements ScriptFile
{
	...
	public _402_PathToKnight()
	{
		...
		addQuestItem(new int[] { BUGBEAR_NECKLACE, EINHASAD_CRUCIFIX, POISON_SPIDER_LEG1, LIZARDMAN_TOTEM, GIANT_SPIDER_HUSK, HORRIBLE_SKULL });
	}
	...
	public String onEvent(String event, QuestState st, NpcInstance npc)
	{
		...
		long squire = st.ownItemCount(MARK_OF_ESQUIRE);
		long coin1 = st.ownItemCount(COIN_OF_LORDS1);
		long coin2 = st.ownItemCount(COIN_OF_LORDS2);
		long coin3 = st.ownItemCount(COIN_OF_LORDS3);
		long coin4 = st.ownItemCount(COIN_OF_LORDS4);
		long coin5 = st.ownItemCount(COIN_OF_LORDS5);
		long coin6 = st.ownItemCount(COIN_OF_LORDS6);
		long guards_mark1 = st.ownItemCount(GLUDIO_GUARDS_MARK1);
		long guards_mark2 = st.ownItemCount(GLUDIO_GUARDS_MARK2);
		long guards_mark3 = st.ownItemCount(GLUDIO_GUARDS_MARK3);
		long church_mark1 = st.ownItemCount(EINHASAD_CHURCH_MARK1);
		long church_mark2 = st.ownItemCount(EINHASAD_CHURCH_MARK2);
		long church_mark3 = st.ownItemCount(EINHASAD_CHURCH_MARK3);
		if(event.equalsIgnoreCase("sir_karrel_vasper_q0402_02a.htm"))
		{
			...
			else if(st.ownItemCount(SWORD_OF_RITUAL) > 0)
	...
	public String onKill(NpcInstance npc, QuestState st)
	{
		int npcId = npc.getNpcId();
		for(int[] element : DROPLIST)
			if(st.getCond() > 0 && npcId == element[0] && st.ownItemCount(element[1]) > 0 && st.ownItemCount(element[2]) < element[3] && Rnd.chance(element[4]))
			{
