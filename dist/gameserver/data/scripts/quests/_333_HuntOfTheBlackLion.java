...
public class _333_HuntOfTheBlackLion extends Quest implements ScriptFile
{
	...
	public String onEvent(String event, QuestState st, NpcInstance npc)
	{
		...
		{
			long claw = st.ownItemCount(LIONS_CLAW) / 10;
			long check_eye = st.ownItemCount(LIONS_EYE);
			if(claw > 0)
			{
				st.giveItems(LIONS_EYE, claw);
				long eye = st.ownItemCount(LIONS_EYE);
		...
		else if(event.equalsIgnoreCase("f_info"))
		{
			...
			if(text < 4)
			{
				st.setMemoState("text", String.valueOf(text + 1));
		...
		{
			if(st.ownItemCount(CARGO_BOX1) > 0)
			{
				if(st.ownItemCount(ADENA_ID) >= OPEN_BOX_PRICE)
				{
		...
		{
			if(st.ownItemCount(COMPLETE_TABLET) > 0)
			{
		...
		{
			if(st.ownItemCount(COMPLETE_STATUE) > 0)
			{
		...
		else if(event.equalsIgnoreCase("m_give"))
		{
			if(st.ownItemCount(CARGO_BOX1) > 0)
			{
				long coins = st.ownItemCount(GUILD_COIN);
				long count = coins / 40;
				if(count > 2)
					count = 2;
	...
	public String onTalk(NpcInstance npc, QuestState st)
	{
		...
		else
		{
			...
			if(npcId == Sophya)
			{
				...
				long box = st.ownItemCount(CARGO_BOX1);
				if(box > 0 && count > 0)
				{
