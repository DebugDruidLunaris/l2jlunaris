...
public class _621_EggDelivery extends Quest implements ScriptFile
{
	...
	public String onEvent(String event, QuestState st, NpcInstance npc)
	{
		...
		int cond = st.getCond();
		long BoiledEgg_count = st.ownItemCount(BoiledEgg);
		...
		if(event.equalsIgnoreCase("jeremy_q0621_0104.htm") && _state == CREATED)
		{
			...
			takeEgg(st, 6);
		else if(event.equalsIgnoreCase("jeremy_q0621_0701.htm") && cond == 6 && st.ownItemCount(FeeOfBoiledEgg) >= 5)
			st.setCond(7);
		else if(event.equalsIgnoreCase("brewer_valentine_q0621_0801.htm") && cond == 7 && st.ownItemCount(FeeOfBoiledEgg) >= 5)
		{
			st.takeItems(BoiledEgg, -1);
			st.takeItems(FeeOfBoiledEgg, -1);
	...
	public String onTalk(NpcInstance npc, QuestState st)
	{
		...
		int cond = st.getCond();
		long BoiledEgg_count = st.ownItemCount(BoiledEgg);
		long FeeOfBoiledEgg_count = st.ownItemCount(FeeOfBoiledEgg);
