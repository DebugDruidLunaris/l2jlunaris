...
public class FourSepulchersManager extends Functions implements ScriptFile, OnDeathListener
{
	...
	private static ScheduledFuture<?> _changeCoolDownTimeTask = null, _changeEntryTimeTask = null, _changeWarmUpTimeTask = null,
	...
	public static void managerSay(int min)
	{
		...
		if(_inAttackTime)
		{
			...
			String msg = min + " минут прошло."; // now this is a proper message^^
			if(min == 90)
				msg = "Игра окончена.Телепорт появлся";
		...
		{
			String msg1 = "Теперь вы можете войти в Гробницу";
			String msg2 = "Если вы положите руку на каменную статую перед каждым гробом," + " Вы сможете войти";
			for(SepulcherNpcInstance npc : FourSepulchersSpawn._managers)
			{
				npc.sayInShout(msg1);
				npc.sayInShout(msg2);
