...
public abstract class TVTArenaTemplate extends Functions
{
	...
	public void template_stop()
	{
		...
		{
			sayToPlayers("Combate interrompido por razões técnicas", true, player, 0);
	...
	public void template_check1(Player player, NpcInstance manager)
	{
		...
		else
		{
			...
			executeTask("events.TVTArena." + _className, "Объявления", new Object[0], 60000);
	...
	private String tryRegister(int team, Player player)
	{
		...
		{
			if(player.getLevel() < _team1min || player.getLevel() > _team1max)
				return "Você não se aproxima do nível de";
			if(_team1list.size() >= _team1count)
				return "Equipe 1 completa";
			doRegister(1, player);
			return null;
		}
		else
		{
			...
			if(_team2list.size() >= _team2count)
				return "В командах осталось свободных мест";
			doRegister(2, player);
		}
		return null;
	}
	...
	private void doRegister(int team, Player player)
	{
		if(team == 1)
		{
			...
			for(Player plr : getPlayers(_team1list))
			{
				sayToPlayers(plr.getName() + " Вы зарегистрировались в команде 1", true, plr, 1);
		...
		else
		{
			...
			for(Player plr : getPlayers(_team2list))
			{
				sayToPlayers(plr.getName() + " Вы зарегистрировались в команде 2", true, plr, 1);
				saveTeamPoints(plr, -1, _nArenaId);
			}
			for(Player plr : getPlayers(_team1list))
			{
		...
		{
			for(Player plr : getPlayers(_team1list))
				sayToPlayers("Все команды готовы, Ивент начинается через 1 минуту.", true, plr, 1);
			for(Player plr : getPlayers(_team2list))
				sayToPlayers("Все команды готовы, Ивент начинается через 1 минуту.", true, plr, 1);
			_timeToStart = 1;
		}
	...
	public void template_announce()
	{
		...
		{
			_timeToStart--;
			String[] param = { creator.getName(), String.valueOf(_nArenaId), String.valueOf(_timeToStart) };
			sayToAll("scripts.events.TVTArena.AnnounceStart", param);
			executeTask("events.TVTArena." + _className, "Объявления", new Object[0], 60000);
		}
		...
		{
			for(Player plr : getPlayers(_team1list))
				sayToPlayers("Подготовка к битве", true, plr, 1);
			for(Player plr : getPlayers(_team2list))
				sayToPlayers("Подготовка к битве", true, plr, 1);
			executeTask("events.TVTArena." + _className, "подготовка", new Object[0], 5000);
		}
		else
		{
			...
			for(Player plr : getPlayers(_team1list))
			{
				sayToPlayers("Бой не Произошел, не было противников", true, plr, 1);
	...
	public void template_prepare()
	{
		...
		executeTask("events.TVTArena." + _className, "начать", new Object[0], 30000);
	}
	...
	public void template_start()
	{
		...
		unParalyzeTeams();
		_status = 3;
		executeTask("events.TVTArena." + _className, "тайм-аут", new Object[0], Config.EVENT_TVT_ARENA_FIGHT_TIME * 60000);
		_timeOutTask = true;
	}
	...
	public void teamHasLost(Integer team_id)
	{
		if(team_id == 1)
		{
			for(Player plr : getPlayers(_team1list))
			{
				sayToPlayers("Команда 2 Выиграла", true, plr, 0);
		...
		else
		{
			for(Player plr : getPlayers(_team1list))
			{
				sayToPlayers("Команда 1 Выиграла", true, plr, 0);
				saveTeamPoints(plr, Config.EVENT_TVT_ARENA_TEAM_WIN, _nArenaId);
			}
			for(Player plr : getPlayers(_team2list))
			{
	...
	public void onDeath(Creature self, Creature killer)
	{
		if(_status >= 2 && self.isPlayer() && self.getTeam() != TeamType.NONE && (_team1list.contains(self.getStoredId()) || _team2list.contains(self.getStoredId())))
		{
			...
			{
				for(Player plr : getPlayers(_team1list))
					sayToPlayers(kplayer.getName() + " убит " + player.getName(), true, plr, 1);
				for(Player plr : getPlayers(_team2list))
					sayToPlayers(kplayer.getName() + " убит " + player.getName(), true, plr, 1);
			...
			switch(_status)
			{
				case 1:
					...
					if(player.getStoredId() == _creatorId)
					{
						for(Player plr : getPlayers(_team1list))
						{
							sayToPlayers("Бой остановлен", true, plr, 0);
