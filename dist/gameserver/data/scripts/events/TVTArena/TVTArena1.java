...
public class TVTArena1 extends Functions implements ScriptFile, OnDeathListener, OnTeleportListener, OnPlayerExitListener
{
	...
	{
		final int EVENT_MANAGERS[][] = { { 82232, 148936, -3492, 0 } };
	...
	public class StartTask extends RunnableImpl
	{
		...
		{
			spawnEventManagers();
			Announcements.getInstance().announceToAll("Evento Iniciado - TVT Arena 1");
	...
	public class StopTask extends RunnableImpl
	{
		...
		public void runImpl()
		{
			...
			Announcements.getInstance().announceToAll("Evento Finalizado - TVT Arena 1");
	...
	public String DialogAppend_36612(Integer val)
	{
		...
		if(val == 0)
			return HtmCache.getInstance().getNotNull("scripts/events/TVTArena/36612.htm", player);
		return "";
	}
	...
	public void showStat()
	{
		...
		playersTopList += "<td valign=\"top\" width=165><center>Jogador</center></td>";
		playersTopList += "<td valign=\"top\" width=50><center>Level</center></td>";
		playersTopList += "<td valign=\"top\" width=50><center>Classificação</center></td>";
		playersTopList += "<td valign=\"top\" width=10></td>";
		playersTopList += "</tr>";
	...
	public void startEvent()
	{
		...
		if(SetActive("TVTArena1", true))
		{
			spawnEventManagers();
			_log.info("Event: TVT Arena 1 started");
			Announcements.getInstance().announceToAll("Ивент - ТВТ Арена 1 стартовал");
		}
		else
	...
	public void stopEvent()
	{
		...
		if(SetActive("TVTArena1", false))
		{
			...
			stop();
			_log.info("Event: TVT Arena 1 stopped");
			Announcements.getInstance().announceToAll("Ивент - ТВТ Арена 1 окончен");
		}
		else
