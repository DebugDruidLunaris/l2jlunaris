...
public class FightClubManager extends Functions implements ScriptFile, OnPlayerExitListener, OnTeleportListener
{
	...
	public void onLoad()
	{
		...
		for(int i = 0; i < Config.FIGHT_CLUB_ALLOWED_RATE_ITEMS.length; i++)
		{
			String itemName = ItemFunctions.createItem(Integer.parseInt(Config.FIGHT_CLUB_ALLOWED_RATE_ITEMS[i])).getTemplate().getName();
			_itemsList.append(itemName).append(";");
			_allowedItems.put(itemName, Integer.parseInt(Config.FIGHT_CLUB_ALLOWED_RATE_ITEMS[i]));
		}
	...
	public static String addApplication(Player player, String item, int count)
	{
		...
		if(Config.FIGHT_CLUB_ANNOUNCE_RATE)
		{
			...
			Announcements.getInstance().announceByCustomMessage("scripts.events.fightclub.Announce", args, ChatType.CRITICAL_ANNOUNCE);
		}
	 ...
	 * 
	 * @param requested
	 *            - игрок, выставивший заявку. <b>Ему</b> отправляется запрос
	 * @param requester
	 *            - игрок, выбравший из списка соперника. <b>От него</b>
	 *            отправляется запрос
	 */
	public static boolean requestConfirmation(Player requested, Player requester)
	{
		...
		if((requested.getLevel() - requester.getLevel()) > Config.FIGHT_CLUB_MAXIMUM_LEVEL_DIFFERENCE || (requester.getLevel() - requested.getLevel()) > Config.FIGHT_CLUB_MAXIMUM_LEVEL_DIFFERENCE)
		{
			show(new CustomMessage("scripts.events.fightclub.CancelledLevel", requester, Config.FIGHT_CLUB_MINIMUM_LEVEL_TO_PARRICIPATION, Config.FIGHT_CLUB_MAXIMUM_LEVEL_TO_PARRICIPATION, Config.FIGHT_CLUB_MAXIMUM_LEVEL_DIFFERENCE), requester);
			return false;
		}
	...
	public static boolean checkPlayer(Player player, boolean first)
	{
		...
		if(player.getLevel() < Config.FIGHT_CLUB_MINIMUM_LEVEL_TO_PARRICIPATION || player.getLevel() > Config.FIGHT_CLUB_MAXIMUM_LEVEL_TO_PARRICIPATION)
		{
	...
	/**
	 * Возвращает объект класса {@link Rate}, содержащий параметры "заявки" <b>
	 * Метод для использования в FightClubInstanceManager! </b>
	 * 
	 * @param index
	...
	/**
	 * Возвращает через ; имена предметов, разрешенных в качестве ставки. <b>
	 * Метод для использования в FightClubInstanceManager! </b>
	 * 
	 ...
	 * {@link #removePlayer(Player)} <b> Метод для использования в
	 * FightClubInstanceManager! </b>
	 * 
	 * @param playerObject
	 *            ссылка на игрока
	 */
	...
	public static void teleportPlayersBack(Player player1, Player player2, Object args)
	{
		...
		if(Config.FIGHT_CLUB_REMOVE_CLAN_SKILLS)
		{
		...
		if(Config.FIGHT_CLUB_REMOVE_HERO_SKILLS)
		{
			if(player1.isHero())
			{
			...
			if(player2.isHero())
			{
				Hero.getInstance().addSkills(player2);
			}
		}
	...
	public static void teleportPlayersToColliseum(Player player1, Player player2, Reflection reflection)
	{
		...
		if(Config.FIGHT_CLUB_UNSUMMON_PETS)
		{
			unSummonPet(player1, true);
			unSummonPet(player2, true);
		}

		if(Config.FIGHT_CLUB_UNSUMMON_SUMMONS)
		{
		...
		if(Config.FIGHT_CLUB_CANCEL_BUFF_BEFORE_FIGHT)
		{
