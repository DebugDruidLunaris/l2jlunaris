...
public class CharInfo extends L2GameServerPacket
{
	...
	public CharInfo(Creature cha)
	{
		...
		if(player.getTransformationName() != null || (player.getReflection() == ReflectionManager.GIRAN_HARBOR || player.getReflection() == ReflectionManager.PARNASSUS) && player.getPrivateStoreType() != Player.STORE_PRIVATE_NONE && !player.isInBuffStore())
		{
		...
		else
		{
			...
			if(player.getPrivateStoreType() != Player.STORE_PRIVATE_NONE && !player.isInBuffStore())
				_title = "";
			else if(!player.isConnected() && !player.isPhantom() && !player.isInBuffStore())
			{
		...
		_hero = player.isFakeHero() || player.isHero() || player.isGM() && Config.GM_HERO_AURA ? 1 : 0; // 0x01: Hero Aura
