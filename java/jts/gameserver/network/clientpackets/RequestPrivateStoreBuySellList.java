		...
		finally
		{
			try
			{
				...
				for(TradeItem si : sellList)
				{
					...
					Log_New.LogEvent(buyer.getName(), buyer.getIP(), "PrivateStore", new String[] { "Bought Item:", "" + item.getName() + " count: " + item.getCount() + " objId: " + item.getObjectId() + " from " + buyer.getName() + "" });
