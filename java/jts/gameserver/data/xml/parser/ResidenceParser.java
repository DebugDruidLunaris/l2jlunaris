...
public final class ResidenceParser extends AbstractDirParser<ResidenceHolder>
{
	...
	protected void readData(Element rootElement) throws Exception
	{
		...
		try
		{
			clazz = Class.forName("jts.gameserver.model.entity.residence." + impl);
			...
			if(nodeName.equalsIgnoreCase("teleport"))
			{
				...
				for(Iterator<Element> it2 = element.elementIterator(); it2.hasNext();)
				{
					...
					if("target".equalsIgnoreCase(teleportElement.getName()))
					{
						...
						String RuName = teleportElement.attributeValue("RuName");
						String EnName = teleportElement.attributeValue("EnName");
						TeleportLocation loc = new TeleportLocation(itemId, price, npcStringId, RuName, EnName, 0);
