...
public class BuyListHolder
{
	...
	private BuyListHolder()
	{
		...
		try
		{
			...
			_log.info("Trade: Carregado " + counterFiles + " file(s).");
			_log.info("Trade: Carregado " + counterItems + " Items.");
			_log.info("Trade: Carregado " + _lists.size() + " Buylists.");
		}
		catch(Exception e)
		{
	...
	public boolean checkItem(ItemTemplate template)
	{
		if(template.isCommonItem() && !Config.ALT_ALLOW_SELL_COMMON)
			return false;
		if(template.isEquipment() && !template.isForPet() && Config.ALT_SHOP_PRICE_LIMITS[0] != 0)
		...
		if(Config.ALT_SHOP_UNALLOWED_ITEMS[0] != 0)
