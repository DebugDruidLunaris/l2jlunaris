...
public class RewardGroup implements Cloneable
{
	...
	public List<RewardItem> roll(RewardType type, Player player, double mod, boolean isRaid, boolean isSiegeGuard)
	{
		...
		switch(type)
		{
			...
			case SWEEP:
				return rollItems(mod * Config.RATE_MOD_DROP_SPOIL, Config.RATE_DROP_SPOIL, player.getRateSpoil());
			case RATED_GROUPED:
				if(_isAdena)
					return rollAdena(mod * Config.RATE_MOD_DROP_ADENA, Config.RATE_DROP_ADENA, player.getRateAdena());

				if(isRaid)
					return rollItems(mod * Config.RATE_MOD_DROP_RAIDBOSS, Config.RATE_DROP_RAIDBOSS, 1.0);

				if(isSiegeGuard)
					return rollItems(mod * Config.RATE_MOD_DROP_SIEGE_GUARD, Config.RATE_DROP_SIEGE_GUARD, SiegeGuard);

				return rollItems(mod * Config.RATE_MOD_DROP_ITEMS, Config.RATE_DROP_ITEMS, player.getRateItems());
			default:
