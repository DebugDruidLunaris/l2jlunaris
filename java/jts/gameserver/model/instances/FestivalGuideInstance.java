...
public final class FestivalGuideInstance extends NpcInstance
{
		...
		else if(command.startsWith("Festival"))
		{
			...
			switch(val)
			{
				...
				case 2: // Festival 2 xxxx
					...
					{
						player.sendMessage(new CustomMessage("jts.gameserver.model.instances.L2FestivalGuideInstance.InProgress", player));
						return;
					}

					// Check if the player is in a formed party already.
					if(playerParty == null || playerParty.getMemberCount() < Config.ALT_FESTIVAL_MIN_PARTY_SIZE)
					{
					...
					long stonesNeeded = (long) Math.floor(SevenSignsFestival.getStoneCount(_festivalType, stoneType) * Config.ALT_FESTIVAL_RATE_PRICE);

					if(!player.getInventory().destroyItemByItemId(stoneType, stonesNeeded))
					{
						player.sendMessage(new CustomMessage("jts.gameserver.model.instances.L2FestivalGuideInstance.NotEnoughSSType", player));
						return;
					}
				...
				case 9: // Leave the Festival
					...
					else if(playerParty.getMemberCount() > Config.ALT_FESTIVAL_MIN_PARTY_SIZE)
						player.leaveParty();
					else
	...
	private void showChatWindow(Player player, int val, String suffix, boolean isDescription)
	{
		...
		html.replace("%min%", String.valueOf(Config.ALT_FESTIVAL_MIN_PARTY_SIZE));
		// If the stats or bonus table is required, construct them.
		if(val == 1)
		{
			html.replace("%price1%", String.valueOf((long) Math.floor(SevenSignsFestival.getStoneCount(_festivalType, 6362) * Config.ALT_FESTIVAL_RATE_PRICE)));
			html.replace("%price2%", String.valueOf((long) Math.floor(SevenSignsFestival.getStoneCount(_festivalType, 6361) * Config.ALT_FESTIVAL_RATE_PRICE)));
			html.replace("%price3%", String.valueOf((long) Math.floor(SevenSignsFestival.getStoneCount(_festivalType, 6360) * Config.ALT_FESTIVAL_RATE_PRICE)));
		}
		if(val == 5)
