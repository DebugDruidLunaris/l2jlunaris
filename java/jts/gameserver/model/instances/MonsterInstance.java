...
public class MonsterInstance extends NpcInstance
{
	...
	public void setChampion()
	{
		if(getReflection().canChampions() && canChampion())
		{
			...
			if(Config.ALT_CHAMPION_CHANCE_RED / 100. >= random)
				setChampion(2);
			else if((Config.ALT_CHAMPION_CHANCE_RED + Config.ALT_CHAMPION_CHANCE_BLUE) / 100. >= random)
				setChampion(1);
			else
				setChampion(0);
		}
	...
	protected void onSpawn()
	{
		...
		if(getMinionList().hasMinions())
		{
			...
			minionMaintainTask = ThreadPoolManager.getInstance().schedule(new MinionMaintainTask(), MONSTER_MAINTENANCE_INTERVAL);
		}
	}
	...
	protected void onDeath(Creature killer)
	{
		...
		if(Config.CAPTCHA_ENABLE)
		{
			if(killer != null && killer.isPlayable())
			{
				...
				if(Rnd.get(0, 100) < Config.CAPTCHA_NPC_CHANCE && player.getCaptchaReguest() < System.currentTimeMillis())
				{
