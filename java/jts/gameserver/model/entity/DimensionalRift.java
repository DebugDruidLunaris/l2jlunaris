...
public class DimensionalRift extends Reflection
{
	...
	{
		if(getParty().getMemberCount() < Config.ALT_RIFT_MIN_PARTY_SIZE || getParty().getMemberCount() == 1 || getPlayersInside(true) == 0)
			createNewKillRiftTimer();
	}
	...
	protected long calcTimeToNextJump()
	{
		...
		return Config.ALT_RIFT_AUTO_JUMPS_TIME * MILLISECONDS_IN_MINUTE + Rnd.get(Config.ALT_RIFT_AUTO_JUMPS_TIME_RAND);
	}
	...
	{
		if(getPlayersInside(false) < Config.ALT_RIFT_MIN_PARTY_SIZE)
			createNewKillRiftTimer();
	}
	...
	{
		return Math.max(Math.min(Config.ALT_RIFT_MAX_JUMPS, 8), 1);
	}
