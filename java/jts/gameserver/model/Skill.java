...
import jts.gameserver.network.serverpackets.FlyToLocation.FlyType;
import jts.gameserver.network.serverpackets.SystemMessage;
import jts.gameserver.network.serverpackets.SystemMessage2;
import jts.gameserver.network.serverpackets.components.SystemMsg;
import jts.gameserver.skills.AddedSkill;
import jts.gameserver.skills.NextAction;
import jts.gameserver.skills.SkillMagicType;
import jts.gameserver.skills.SkillOpType;
import jts.gameserver.skills.SkillTargetType;
import jts.gameserver.skills.SkillType;
import jts.gameserver.skills.Ternary;
...
public abstract class Skill extends StatTemplate implements Cloneable
{
	...
	public final Creature getAimingTarget(Creature activeChar, GameObject obj)
	{
		...
		switch(_targetType)
		{
			...
			case TARGET_CORPSE:
				...
				if (target.isSummon() && target != activeChar.getPet()) // РёСЃРїРѕР»СЊР·РѕРІР°С‚СЊ СЃРѕР±СЃС‚РІРµРЅРЅРѕРіРѕ РјРµСЂС‚РІРѕРіРѕ СЃР°РјРѕРЅР° РЅРµР»СЊР·СЏ
					return target;
	...
	public List<Creature> getTargets(Creature activeChar, Creature aimingTarget, boolean forceUse)
	{
		...
		switch(_targetType)
		{
			...
			case TARGET_ALLY:
			{
				...
				if(_targetType != SkillTargetType.TARGET_PARTY_NOT_SELF)
				{
	...
	public final void getEffects(Creature effector, Creature effected, boolean calcChance, boolean applyOnCaster, boolean skillReflected)
	{
		...
		if(isMusic())
			timeMult = Config.ALT_SONGDANCETIME_MODIFIER;
		else if(getId() >= 4342 && getId() <= 4360)
			timeMult = Config.ALT_CLANHALL_BUFFTIME_MODIFIER;
