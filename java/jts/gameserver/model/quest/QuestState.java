package jts.gameserver.model.quest;
...
import jts.gameserver.network.serverpackets.ExShowQuestMark;
import jts.gameserver.network.serverpackets.PlaySound;
import jts.gameserver.network.serverpackets.QuestList;
import jts.gameserver.network.serverpackets.SystemMessage2;
import jts.gameserver.network.serverpackets.TutorialEnableClientEvent;
import jts.gameserver.network.serverpackets.TutorialShowHtml;
import jts.gameserver.network.serverpackets.TutorialShowQuestionMark;
...
public final class QuestState
{
  ...
  public long getQuestItemsCount(int... itemsIds)
  {
	 ...
	 * @return int
	 */
	public long ownItemCount(int itemId)
	{
		Player player = getPlayer();
		return player == null ? 0 : player.getInventory().getCountOf(itemId);
	}

	public long ownItemCount(int... itemsIds)
	{
		long result = 0;
		for(int id : itemsIds)
  ...
  public boolean haveQuestItem(int itemId, int count)
  {
	...
	public boolean rollAndGive(int itemId, int min, int max, int limit, double calcChance)
	{
		...
		if(count > 0)
		{
			long alreadyCount = ownItemCount(itemId);
	...
	public String setMemoState(String var, String val)
	{
		return setMemoState(var, val, true);
	}

	public String setMemoState(String var, int intval)
	{
	 ...
	 */
	public String removeMemo(String var)
	{
		if(var == null) 
		{
			return null;
