...
import jts.commons.logging.LogUtils;
import jts.commons.threading.RunnableImpl;
import jts.commons.util.TroveUtils;
...
import jts.gameserver.network.serverpackets.ExNpcQuestHtmlMessage;
...
public class Quest
{
	...
	/**
	 * Добавляет нпц масив для слушателя при их убийстве, и обновлении пакетом {@link jts.gameserver.network.serverpackets.ExQuestNpcLogList}
	 * @param cond
	 * @param varName
	 * @param killIds
	 */
	...
	public String getDescr(Player player)
	{
		...
		int state = 2;
		if(qs == null || qs.isCreated() && qs.isDailyQuest())
			state = 1;
		else if(qs.isCompleted() || !qs.isDailyQuest())
			state = 3;
