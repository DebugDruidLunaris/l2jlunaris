...
public class AugmentationData
{
	...
	private int generateRandomWeaponAugmentation(int lifeStoneLevel, int lifeStoneGrade)
	{
		...
		{
			case 0:
				generateSkill = Rnd.chance(Config.ALT_AUGMENTATION_NG_SKILL_CHANCE);
				generateGlow = Rnd.chance(Config.ALT_AUGMENTATION_NG_GLOW_CHANCE);
				break;
			case 1:
				generateSkill = Rnd.chance(Config.ALT_AUGMENTATION_MID_SKILL_CHANCE);
				generateGlow = Rnd.chance(Config.ALT_AUGMENTATION_MID_GLOW_CHANCE);
				break;
			case 2:
				generateSkill = Rnd.chance(Config.ALT_AUGMENTATION_HIGH_SKILL_CHANCE);
				generateGlow = Rnd.chance(Config.ALT_AUGMENTATION_HIGH_GLOW_CHANCE);
				break;
			case 3:
				generateSkill = Rnd.chance(Config.ALT_AUGMENTATION_TOP_SKILL_CHANCE);
				generateGlow = Rnd.chance(Config.ALT_AUGMENTATION_TOP_GLOW_CHANCE);
				break;
		}

		if(!generateSkill && Rnd.get(1, 100) <= Config.ALT_AUGMENTATION_BASESTAT_CHANCE)
	...
	private int generateRandomAccessoryAugmentation(int lifeStoneLevel, int bodyPart)
	{
		...
		if(Rnd.get(1, 100) <= Config.ALT_AUGMENTATION_ACC_SKILL_CHANCE)
		{
	...
	public TriggerInfo getAugmentTrigger(int att)
	{
		// TODO Доделать возврат названия скила аугментации. (Для коммисссионного магазина в кб)
		return null;
	}
}