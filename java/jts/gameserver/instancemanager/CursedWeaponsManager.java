...
{
	public static final Logger _log = LoggerFactory.getLogger(CursedWeaponsManager.class);
		...
		try
		{
			...
			while(rset.next())
			{
				...
				if(cw != null)
				{
					...
					// Обновляем скилл и статы игрока, который владеет проклятым оружием. Возможно здесь не нужно.
